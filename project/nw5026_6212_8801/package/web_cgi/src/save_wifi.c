...
int get_wifi_module(char *module)
{
	char uci_option_str[64]="\0";
	ctx=uci_alloc_context();
	strcpy(uci_option_str,"wireless2.@wifi[0].wifi_module"); 
	uci_get_option_value(uci_option_str,module);
	memset(uci_option_str,'\0',64);
	// d_printf("wifimode=%s\n", mode);
...
int main()
{
	...
	sleep(15);
	get_wifi_module(wifi_module);
	if ( !strcmp(wifi_module, "AP6212") )	
		client_sta= check_connected_sta_6212();
	else if ( !strcmp(wifi_module, "MRVL8801") )
		client_sta= check_connected_sta_8801();
	
	if(client_sta==FALSE)
	{
		d_printf("client is not connected.\n");
		//system("control_dns.sh &");
		return 1;
	}
