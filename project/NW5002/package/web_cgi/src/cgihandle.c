...
int getDMS(xmlNodePtr tag, char *retstr)
{
	...
	//uci_get_option_value(uci_option_str,path);
	//memset(uci_option_str,'\0',64);
	sprintf(retstr,"<DMS name=\"%s\" status=\"%s\" path=\"%s\" enable=\"OFF\"></DMS>",name,status,path);
	return 0;
}
...
int getVersion(xmlNodePtr tag, char *retstr)
{
	sprintf(retstr,"<Version fw1=\"%s\" fw2=\"%s\"></Version>",FW_1,FW_2);
	return 0;
}
...
int getpower(xmlNodePtr tag, char *retstr)
{
	...
	unsigned char bat;
	int fh=NULL;
	sprintf(retstr,"<Return status=\"true\">UNSUPPORT!</Return>");
	return; 
...
int setssid(xmlNodePtr tag, char *retstr)  //network restart
{
	...
	//name="valueStr"
	//valueStr=xmlGetProp(tag,(const xmlChar*)"name");
	mylogstrTofile("aaaaaaaaaaaaaaaa");
	...
	}mylogstrTofile("bbbbbbbb");
	printf("Content-type:text/html\r\n\r\n");
	printf("<?xml version=\"1.0\" encoding=\"utf-8\"?>");
	printf("<%s><Return status=\"true\" delay=\"5\"></Return></%s>\r\n",SETSYSSTR,SETSYSSTR);
	fflush(stdout);mylogstrTofile("ccccccccccc");
	...
	if(strlen(name))
	{
		sprintf(str_sp,"uci set wireless.@wifi-iface[0].ssid=\"%s\"",name);mylogstrTofile("5555555 %s name=%s\n",str_sp,name);
		system(str_sp);//uci_set_option_value(str_sp);mylogstrTofile("444 %s name=%s\n",str_sp,name);
		memset(str_sp,0,64);
...
//		system(str_sp);
	...
	if(!strcmp(encrypt_config,"none") || !strcmp(encrypt_config,"wep"))
	{
		...
		system(str_sp);//uci_set_option_value(str_sp);mylogstrTofile("333 %s\n",str_sp);
		memset(str_sp,0,64);
...
//		system(str_sp);
	...
	else if(!strcmp(encrypt_config,"psk") || !strcmp(encrypt_config,"psk2") || !strcmp(encrypt_config,"mixed-psk"))
	{
		if(strlen(tkip_aes_config)>0)
		{
			...
			system(str_sp);//uci_set_option_value(str_sp);mylogstrTofile("222 %s\n",str_sp);
			memset(str_sp,0,64);
...
//			system(str_sp);
	...
	if(password!=NULL)
	{
		if(strlen(password))
		{
			...
			system(str_sp);//uci_set_option_value(str_sp);mylogstrTofile("1111 %s\n",str_sp);
			memset(str_sp,0,64);
...
int setJoinWireless(xmlNodePtr tag, char *retstr)  //wlan1 disabled=0;network restart
{
	...
	char uci_option_str[64]="\0";
	char WiredMode[10]="\0";
	mylogstrTofile("%s",retstr);

	strcpy(uci_option_str,"network.wan.proto");  //cur_mode
	uci_get_option_value(uci_option_str,WiredMode);
	memset(uci_option_str,'\0',64);
	
	if(0)//strcmp(WiredMode,"dhcp"))//static ip or pppoe,reset to dhcp
	{
		...
		system("uci set network.wan.hostname=BPEN");
	}
...
*/	
	...
	if(strlen(name))
	{
		...
		sprintf(str_sp_wifilist,"wifisavelist.@wifi-iface[50].ssid=%s",name);
		uci_set_option_value(str_sp);
		uci_set_option_value(str_sp_wifilist);
		memset(str_sp,0,64);
		sprintf(str_sp,"nor set wwanssid=\"%s\"",name);
		system(str_sp);
		memset(str_sp,0,64);
	...
	if(setencryflag==1)
	{
		sprintf(str_sp,"nor set wwanencry=%s",encrypt_config);
		system(str_sp);
		memset(str_sp,0,64);	
	}
	else if(setencryflag==2)
	{
		sprintf(str_sp,"nor set wwanencry=%s+%s",encrypt_config,tkip_aes_config);
		system(str_sp);
		memset(str_sp,0,64);	
	}
	if(password!=NULL)
	{
		if(strlen(password))
		{
			...
			sprintf(str_sp_wifilist,"wifisavelist.@wifi-iface[50].key=%s",password);
			uci_set_option_value(str_sp);
			uci_set_option_value(str_sp_wifilist);
			memset(str_sp,0,64);
		    sprintf(str_sp,"nor set wwanpwd=\"%s\"",password);
		    system(str_sp);
		    memset(str_sp,0,64);
			memset(str_sp_wifilist,0,64);
			free(name);
	...
	system("uci set wifisavelist.@wifi-iface[50].num=51");
	if(channel!=NULL)
	{
		if(strlen(channel))
		{
		    ...
		    sprintf(str_sp,"nor set wwanchan=%s",channel);
		    system(str_sp);
		    memset(str_sp,0,64);	
		}
#if 0
...
int setSAMBA(xmlNodePtr tag, char *retstr)
{
	...
	if((pxml=xmlGetprop(tag,(const xmlChar*)SAMBA_SET_enable))!=NULL)
	{
		...
		sprintf(str_sp,"samba.@samba[0].enabled=%s",enable_config);
		uci_set_option_value(str_sp);
		memset(str_sp,0,64);
	...
	if((pxml=xmlGetprop(tag,(const xmlChar*)SAMBA_SET_password))!=NULL)
	{
		...
		sprintf(str_sp,"(echo %s;echo %s) | smbpasswd -s pisen >/dev/null",password,password);
		system(str_sp);
		memset(str_sp,0,64);
...
//		system(str_sp);
	...
	if( (pxml=xmlGetprop(tag,(const xmlChar*)SAMBA_SET_anonymous_en))!= NULL)
	{
		...
		else
		{
			...
			strcpy(str_sp,"samba.@sambashare[0].users=pisen");
			uci_set_option_value(str_sp);
			memset(str_sp,0,64);
		}
...
out:
	...
	memset(uci_option_str,0,64);
	sprintf(uci_option_str,"nor set airplayname=\"%s\"",name);
