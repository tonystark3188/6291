...
int SocketUartServerStart()
{
	...
	listen_fd = DM_InetServerInit(AF_INET, SOCKET_UART_PORT, SOCK_STREAM, SOCKET_UART_WAIT_COUNT);
	if(listen_fd < 0)
	{
	...
	while(1)
	{
		...
		error_code = 0;
		accept_fd = DM_ServerAcceptClient(listen_fd);
		if(accept_fd < 0)
		{
		...
		ret = DM_MsgReceive(accept_fd, &rev_buf, SOCKET_UART_TIMEOUT);
		if(ret < 0)
		{
		...
		ret = DM_MsgSend(accept_fd, send_buf, send_len);
		if(ret < 0)
		{
...
int SocketUartClientStart(socket_uart_cmd_t *p_socket_uart_cmd)
{
	...
	//debug("send_buf = %s\n", send_buf);
	client_fd = DM_InetClientInit(AF_INET, SOCKET_UART_PORT, SOCK_STREAM, local_ip);
	if(client_fd < 0)
	{
	...
	send_len = strlen(send_buf);	
	ret = DM_MsgSend(client_fd, send_buf, send_len);	
	if(ret < 0)
	{
	...
	ret = DM_MsgReceive(client_fd, &rev_buf, time_out);
	if(ret < 0)
	{
		if(ret = MSGRET_TIMED_OUT)
		{
