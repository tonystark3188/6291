...
# generate an ext2 fs
	...
	# generate an ext2 fs for configs 4MB 4MB
	$(STAGING_DIR_HOST)/bin/genext2fs -U -B 4096 -b 256 -N 8 -d $(TOPDIR)/package/configs/factory $(KDIR)/factory.ext4 -m $(CONFIG_TARGET_EXT4_RESERVED_PCT) $(MKFS_DEVTABLE_OPT)
	$(STAGING_DIR_HOST)/bin/genext2fs -U -B 4096 -b 256 -N 8 -d $(TOPDIR)/package/configs/user $(KDIR)/user.ext4 -m $(CONFIG_TARGET_EXT4_RESERVED_PCT) $(MKFS_DEVTABLE_OPT)
#	$(STAGING_DIR_HOST)/bin/genext2fs -U -B 4096 -b 25600 -N 1024 -d $(TOPDIR)/package/configs/empty $(KDIR)/empty.ext4 -m $(CONFIG_TARGET_EXT4_RESERVED_PCT) $(MKFS_DEVTABLE_OPT)
# convert it to ext4
	$(STAGING_DIR_HOST)/bin/tune2fs $(if $(CONFIG_TARGET_EXT4_JOURNAL),-j) -O extents,uninit_bg,dir_index $(KDIR)/root.ext4
	$(STAGING_DIR_HOST)/bin/tune2fs $(if $(CONFIG_TARGET_EXT4_JOURNAL),-j) -O extents,uninit_bg,dir_index $(KDIR)/factory.ext4
	$(STAGING_DIR_HOST)/bin/tune2fs $(if $(CONFIG_TARGET_EXT4_JOURNAL),-j) -O extents,uninit_bg,dir_index $(KDIR)/user.ext4
#	$(STAGING_DIR_HOST)/bin/tune2fs $(if $(CONFIG_TARGET_EXT4_JOURNAL),-j) -O extents,uninit_bg,dir_index $(KDIR)/empty.ext4
# fix it up
	$(STAGING_DIR_HOST)/bin/e2fsck -fy $(KDIR)/root.ext4
	$(STAGING_DIR_HOST)/bin/e2fsck -fy $(KDIR)/factory.ext4
	$(STAGING_DIR_HOST)/bin/e2fsck -fy $(KDIR)/user.ext4
#	$(STAGING_DIR_HOST)/bin/e2fsck -fy $(KDIR)/empty.ext4
...
define Image/Checksum
	( cd ${BIN_DIR} ; \
		$(STAGING_DIR_HOST)/bin/final_fw ${BIN_DIR}/openwrt-xburst-canna-uImage.bin ${BIN_DIR}/openwrt-xburst-canna-root.jffs2-32k \
		${BIN_DIR}/final_fw.bin ; \
