...
static int process_noa_cfg(int argc, char *argv[]);
#endif
#endif
...
process_mgmtframetx(int argc, char *argv[])
{
	...
	t_u8 peer_mac[ETH_ALEN];
	t_u16 data_len = 0, type = 0, subtype = 0, seq_ctl = 0;
		...
		if (strcmp(args[0], "PktType") == 0) {
			...
			for (i = 0; i < arg_num - 1; i++)
				pmgmt_frame->payload[i] =
					(t_u8)A2HEXDECIMAL(args[i + 1]);
			data_len += arg_num - 1;
		} else if (strcmp(args[0], "SeqCtl") == 0) {
			seq_ctl = (t_u16)A2HEXDECIMAL(args[1]);
			pmgmt_frame->seq_ctl = seq_ctl << 4;
		}
	}
 ...
 */
static int
process_noa_cfg(int argc, char *argv[])
{
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd;
	struct ifreq ifr;
	...
	if (argc == 3) {
		noa_cfg = (mlan_ds_wifi_direct_config *)buffer;
		if (!noa_cfg) {
