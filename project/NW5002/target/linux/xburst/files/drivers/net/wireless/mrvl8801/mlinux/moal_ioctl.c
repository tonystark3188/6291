...
#define GET_FACTORY_MAC_ENABLE 1
...
unsigned char Char2Hex(const char c)
{
	...
	case 'A'...'F':
		return 0xa + (c-'A');
	default:
		printk("INVALID VALUE in %s\n",__FUNCTION__);
		return 0;
	}
}
//convert a string,which length is 12, to a macaddress data type

#define STR2MAC(mac,str)  \
	int i = 0; \
	do { \
		for(i = 0; i < MAC_LEN_IN_BYTE; i++) {\
  			mac[i] = (Char2Hex(str[i*2]) << 4) + Char2Hex(str[i*2 + 1]);\
		}\
	} while(0)

static use_default_mac(unsigned char *buf)
{
	unsigned char temp_mac[32]={0};

	printk(">>>>>>>>>use_default_mac\n");
	/*use default mac*/
	strcpy(temp_mac,"845dd7aabbcc");
		
	if((strlen(temp_mac) != 12))
	{
		printk("ERROR! get_factory_mac invalid temp_mac=%s\n ",temp_mac);
		return -1;
	}
	memset(buf,0x0,strlen(buf));
	STR2MAC(buf,temp_mac);
	return 0;

}

static int get_factory_mac(unsigned char *buf)
{
    struct file *fp = NULL;
    mm_segment_t fs;
    unsigned char source_addr[18]={0};

    loff_t pos = 0;		 
    unsigned char *head, *end; 		 
    int i = 0; 		 

	fp = filp_open("/etc/mac.txt", O_RDONLY,  0444);
    if (IS_ERR(fp))		 
    {			 
       printk("Can not access wifi mac file : /etc/mac.txt\n");
	   return use_default_mac(buf);
    }		 
    else		 
 	{					 
 	    fs = get_fs();
	    ...
	    source_addr[17] = ':';//printk("source_addr %s \n", source_addr);
...
woal_request_set_mac_address(moal_private *priv)
{
...
#ifdef GET_FACTORY_MAC_ENABLE
    get_factory_mac(priv->current_addr);
#endif
	...
	if (status == MLAN_STATUS_SUCCESS) {
		...
		PRINTM(MERROR,
		       "11 set mac address failed! status=%d, error_code=0x%x\n",
		       status, req->status_code);
		printk("aa set mac address failed!\n");
	}
done:
