...
{   
	...
	res = handle_backfile_special_delete_cmd(pdel_data->flist,uuid);
	if(res < 0)
	{
        DMCLOG_D("back rm failed, ret(0x%x)", res);
        EXIT_FUNC();
		goto EXIT;
    }
...
{   
	...
	{
		*file_path = (char *)calloc(1,strlen(get_db_opr_obj(msg).data.backup_file_data.path) + 1);
