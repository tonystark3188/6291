...
int db_write_task_msg_cb(void *self, void *msg)
{
	...
	{
		ret = db_delete_backup_file(database, &p_OprObj->data.backup_file_data);
	}
	else if(p_obj->m_type == MSG_DB_BACKUP_FILE_SPECIAL_DEL)
	{
		ret = db_special_delete_backup_file(database, &p_OprObj->data.backup_file_data);
	}
