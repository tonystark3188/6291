...
static int use_default_mac(unsigned char *buf)
{
	unsigned char temp_mac[32]={0};

	printf(">>>>>>>>>use_default_mac\n");
	/*use default mac*/
	strcpy(temp_mac,"845dd7aabbcc");
		
	if((strlen(temp_mac) != 12))
	{
		printf("ERROR! get_factory_mac invalid temp_mac=%s\n ",temp_mac);
		return -EINVAL;
	}
...
int wifi_platform_get_mac_addr(wifi_adapter_info_t *adapter, unsigned char *buf)
{
	...
	DHD_ERROR(("%s\n", __FUNCTION__));
#ifdef GET_FACTORY_MAC_ENABLE
	printk(">>>>>>>>>>>>>>>>> my self function\n");
	if(buf)
	{
        return get_factory_mac(buf);
	}
	else
	{
	    printf("ERROR buf == NULL in %s %d\n",__FUNCTION__,__LINE__);
	    return -EOPNOTSUPP;
	}
#else
	printk(">>>>>>>>>>>>>>>>> bcm driver function\n");
