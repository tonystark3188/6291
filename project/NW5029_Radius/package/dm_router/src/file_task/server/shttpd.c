...
stop_stream(struct stream *stream)
{
	...
	if(!(stream->conn->cmd == FN_FILE_UPLOAD||stream->conn->cmd == FN_FILE_STREAM_UPLOAD|| stream->conn->cmd == FN_FILE_GET_BACKUP_FILE || stream->conn->cmd == FN_ROUTER_SET_UPLOAD_FIRMWARE))
	{
...
write_stream(struct stream *from, struct stream *to)
{
	...
	if (n > 0)
	{
		...
		if(!(from->conn->cmd == FN_ROUTER_SET_UPLOAD_FIRMWARE||from->conn->cmd == FN_FILE_UPLOAD||from->conn->cmd == FN_FILE_STREAM_UPLOAD||from->conn->cmd == FN_FILE_GET_BACKUP_FILE))
			to->io.total += n;
	}
...
read_stream(struct stream *stream)
{
	...
	if (n > 0)
	{
		...
		if(stream->content_len == stream->io.total&&(stream->conn->cmd == FN_ROUTER_SET_UPLOAD_FIRMWARE||stream->conn->cmd == FN_FILE_UPLOAD||stream->conn->cmd == FN_FILE_GET_BACKUP_FILE))
		{
			...
			char *buf = dm_file_inotify(stream->conn->cmd,stream->conn->error);
	...
	{
		if((stream->conn->cmd == FN_FILE_GET_DIR_BY_TYPE\
			||stream->conn->cmd == FN_FILE_GET_LSIT_BY_TYPE\
			||stream->conn->cmd == FN_FILE_GET_DIR_LIST_BY_TYPE\
			||stream->conn->cmd == FN_FILE_GET_ALBUM_LIST\
			||stream->conn->cmd == FN_FILE_GET_LIST_BY_PATH\
			||stream->conn->cmd == FN_FILE_GET_LIST)&&n == 0)
		{
			n = n;
		}else{
			if(stream->conn->cmd == FN_FILE_COPY)
			{
				if(stream->conn->copy_status != NULL)
				{
					*stream->conn->copy_status = -1;
					DMCLOG_E("quit copy process");
				}
			}
			stop_stream(stream);
		}
...
disconnect(struct conn *c)
{
	...
	if (dont_close) {
			...
			if(c->dn == NULL)
			{
				...
				if(c->cmd != FN_ROUTER_SET_UPLOAD_FIRMWARE)
				{
					if(c->cmd == FN_FILE_GET_BACKUP_FILE)
					{
						...
						c->statusFlag = 3;//backup file finish
						DMCLOG_D("c->statusFlag = %d",c->statusFlag);
						//c->statusFlag = 2;//backup file start
					}else if(c->cmd = FN_FILE_UPLOAD){
					
						DMCLOG_D("c->modifyTime = %lu", c->modifyTime);
						if(c->modifyTime != 0)
						{
							struct utimbuf timebuf;
							timebuf.actime = c->modifyTime;//访问时间
							timebuf.modtime = c->modifyTime;//修改时间
							if(0 == utime(c->des_path, &timebuf))
							{
								DMCLOG_D("set time success");
							}
		...
		if(c->error == 0)
		{
			...
			update_session_time(c->session,&c->ctx->p_session_list);
#endif
			if(c->cmd == FN_FILE_RENAME\
				||c->cmd == FN_FILE_UPLOAD\
				|| c->cmd == FN_FILE_DELETE\
				|| c->cmd == FN_FILE_DELETE_ALL\
				|| c->cmd == FN_FILE_COPY\
				|| c->cmd == FN_FILE_MOVE\
				|| c->cmd == FN_FILE_GET_BACKUP_FILE\
		...
		if (/*c->cmd != FN_FILE_DOWNLOAD&&*/c->rem.io_class != NULL)
...
shttpd_poll(struct shttpd_ctx *ctx, int milliseconds)
{
	...
	LL_FOREACH(&ctx->connections, lp) {
		...
		if ((io_space_len(&c->loc.io) && (c->loc.flags & FLAG_R) &&
		    (c->loc.flags & FLAG_ALWAYS_READY))||c->cmd == FN_FILE_DELETE_ALL_CB)
			msec = 0;	
