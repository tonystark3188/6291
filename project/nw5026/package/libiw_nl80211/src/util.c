...
void print_ampdu_length(__u8 exponent)
{
	...
	if (max_ampdu_length) {
		p_debug("\t\tMaximum RX AMPDU length %d bytes (exponent: 0x0%02x)\n",
		       max_ampdu_length, exponent);
	} else {
		p_debug("\t\tMaximum RX AMPDU length: unrecognized bytes "
		       "(exponent: %d)\n", exponent);
	}
}
...
{
	p_debug("\t\tMinimum RX AMPDU time spacing: %s (0x%02x)\n",
...
void print_ht_mcs(const __u8 *mcs)
{
	...
	if (max_rx_supp_data_rate)
		p_debug("\t\tHT Max RX data rate: %d Mbps\n", max_rx_supp_data_rate);
	/* XXX: else see 9.6.0e.5.3 how to get this I think */

	if (tx_mcs_set_defined) {
		if (tx_mcs_set_equal) {
			p_debug("\t\tHT TX/RX MCS rate indexes supported:");
			print_mcs_index(mcs);
		} else {
			p_debug("\t\tHT RX MCS rate indexes supported:");
			print_mcs_index(mcs);

			if (tx_unequal_modulation)
				p_debug("\t\tTX unequal modulation supported\n");
			else
				p_debug("\t\tTX unequal modulation not supported\n");

			p_debug("\t\tHT TX Max spatial streams: %d\n",
				tx_max_num_spatial_streams);

			p_debug("\t\tHT TX MCS rate indexes supported may differ\n");
		}
	} else {
		p_debug("\t\tHT RX MCS rate indexes supported:");
		print_mcs_index(mcs);
		p_debug("\t\tHT TX MCS rate indexes are undefined\n");
	}
}
...
void print_vht_info(__u32 capa, const __u8 *mcs)
{
	...
	p_debug("\t\tVHT Capabilities (0x%.8x):\n", capa);
...
#define PRINT_VHT_CAPA(_bit, _str) \
	...
	p_debug("\t\t\tMax MPDU length: ");
	switch (capa & 3) {
	case 0: p_debug("3895\n"); break;
	case 1: p_debug("7991\n"); break;
	case 2: p_debug("11454\n"); break;
	case 3: p_debug("(reserved)\n");
	}
	p_debug("\t\t\tSupported Channel Width: ");
	switch ((capa >> 2) & 3) {
	case 0: p_debug("neither 160 nor 80+80\n"); break;
	case 1: p_debug("160 MHz\n"); break;
	case 2: p_debug("160 MHz, 80+80 MHz\n"); break;
	case 3: p_debug("(reserved)\n");
	}
	...
	p_debug("\t\tVHT RX MCS set:\n");
	tmp = mcs[0] | (mcs[1] << 8);
	for (i = 1; i <= 8; i++) {
		p_debug("\t\t\t%d streams: ", i);
		switch ((tmp >> ((i-1)*2) ) & 3) {
		case 0: p_debug("MCS 0-7\n"); break;
		case 1: p_debug("MCS 0-8\n"); break;
		case 2: p_debug("MCS 0-9\n"); break;
		case 3: p_debug("not supported\n"); break;
		}
	}
	tmp = mcs[2] | (mcs[3] << 8);
	p_debug("\t\tVHT RX highest supported: %d Mbps\n", tmp & 0x1fff);

	p_debug("\t\tVHT TX MCS set:\n");
	tmp = mcs[4] | (mcs[5] << 8);
	for (i = 1; i <= 8; i++) {
		p_debug("\t\t\t%d streams: ", i);
		switch ((tmp >> ((i-1)*2) ) & 3) {
	...
	tmp = mcs[6] | (mcs[7] << 8);
	p_debug("\t\tVHT TX highest supported: %d Mbps\n", tmp & 0x1fff);
}
