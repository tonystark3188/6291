...
#define PARSE_FLAG(nl_flag, string_value)  do { \
	...
	nla_for_each_nested(nl_rule, tb_msg[NL80211_ATTR_REG_RULES], rem_rule) {
		...
		if (MBI_TO_DBI(max_ant_gain_mbi))
			p_debug("%d", MBI_TO_DBI(max_ant_gain_mbi));
		else
			p_debug("N/A");

		p_debug(", %d)", MBM_TO_DBM(max_eirp_mbm));

		if ((flags & NL80211_RRF_DFS) && tb_rule[NL80211_ATTR_DFS_CAC_TIME])
			p_debug(", (%u ms)", nla_get_u32(tb_rule[NL80211_ATTR_DFS_CAC_TIME]));
		else
			p_debug(", (N/A)");
