...
#	if [ $boot_flag = "04" ];then
	if [ -f /tmp/in_backup_system ]; then
		...
		if [ $sys_flag = "00" ]; then
			...
			sync;sync;sync
			dd if=/tmp/fwupgrade of=/dev/mmcblk0 bs=1M skip=5 seek=13 conv=fsync
			[ $? -ne 0 ] && {
				v "Upgrade failed"
				exit
			}
			...
			sync;sync;sync
			dd if=/tmp/fwupgrade of=/dev/mmcblk0 bs=1M skip=5 seek=134 conv=fsync
			[ $? -ne 0 ] && {
				v "Upgrade failed"
				exit
			}
...
#	/tmp/set_sys_flag auto
	if [ -f /tmp/dont_reboot ]; then
		echo timer > /sys/class/leds/wifi\:led/trigger
		echo 100 > /sys/class/leds/wifi\:led/delay_on
		echo 100 > /sys/class/leds/wifi\:led/delay_off
