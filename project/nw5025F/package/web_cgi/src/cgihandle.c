...
int setssid(xmlNodePtr tag, char *retstr)  //network restart
{
	...
	if(strlen(name))
	{
		...
		// nor_set(str_sp,name);
		sprintf(str_sp,"nor set ssid_name=%s",name);
		system(str_sp);
		memset(str_sp,0,64);
	}
	...
	if(password!=NULL && strcmp(encrypt_config,"none")!=0)
	{
		if(strlen(password))
		{
...
//		system(str_sp);
	...
	if( (pxml=xmlGetprop(tag,(const xmlChar*)SAMBA_SET_anonymous_en))!= NULL)
	{
		...
		else
		{
			...
			strcpy(str_sp,"samba.@sambashare[0].users=airdisk");
			uci_set_option_value(str_sp);
			memset(str_sp,0,64);
		}
