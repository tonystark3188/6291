		...
		{ 
			...
			if(0 == memcmp(mtpair->mnt_dir, MOUNT_PATH, strlen(MOUNT_PATH)))
			{
				...
				if (disk_info->spath != NULL&&statfs(disk_info->spath, &fs) == 0)
				{
					...
					DMCLOG_D("disk_info->fstype: %s", disk_info->fstype);
				}
...
int dm_get_storage (all_disk_t *alldisk)
{
	...
	while((ptr = readdir(dir))!=NULL)
	{
		if((ptr->d_name[0]=='s')&&(ptr->d_name[1]=='d')&&strlen(ptr->d_name) >= 3)
		{
			...
			sprintf(device, "/dev/%s", ptr->d_name);
			if(dev_is_hd(device) == 0)
			{
	...
	while((ptr = readdir(dir))!=NULL)
	{
		...
		{
			//DMCLOG_D("ptr->name1 = %s",ptr->d_name);
			ret = get_name_from_dev(ptr->d_name,&alldisk->disk[alldisk->count]);
			if(ret < 0)
			{
				//DMCLOG_D("ptr->name2 = %s",ptr->d_name);
				continue;
			}
			...
			if(alldisk->disk[alldisk->count].spath)
			{
				...
				//alldisk->disk[alldisk->count].type = get_dev_type(fulldev);
				//DMCLOG_D("%s, %s, %s, %9lu, %9lu, %d", alldisk->disk[alldisk->count].name, alldisk->disk[alldisk->count].path,
					//alldisk->disk[alldisk->count].dev, alldisk->disk[alldisk->count].total_size, alldisk->disk[alldisk->count].free_size);
				alldisk->count++;
			}
			else
			{
				...
				if(strlen(ptr->d_name) > 3)
				{
					if(dev_is_hd(device) == 0)
					{
						alldisk->disk[alldisk->count].is_format = 0;
						sprintf(alldisk->disk[alldisk->count].name, "%s%c", HDISK_NAME, ptr->d_name[3]);
						strcpy(alldisk->disk[alldisk->count].path, "/tmp/mnt/");
						strcat(alldisk->disk[alldisk->count].path, alldisk->disk[alldisk->count].name);
						strcpy(alldisk->disk[alldisk->count].dev, ptr->d_name);
						alldisk->disk[alldisk->count].total_size = alldisk->disk[alldisk->count].free_size = 0;
						sprintf(fulldev, "/dev/%s", ptr->d_name);
						alldisk->disk[alldisk->count].type = get_dev_type(fulldev);
						/*printf("%s, %s, %s, %9lu, %9lu, %d\n", alldisk->disk[alldisk->count].name, alldisk->disk[alldisk->count].path,
							alldisk->disk[alldisk->count].dev, alldisk->disk[alldisk->count].total_size, alldisk->disk[alldisk->count].free_size);*/
						alldisk->count++;
					}
				}
				else if(strlen(ptr->d_name) == 3)
				{
					if((dev_is_hd(device) == 0) && (hd_disk_cnt == 0))
					{
						alldisk->disk[alldisk->count].is_format = 0;
						sprintf(alldisk->disk[alldisk->count].name, "%s%c", HDISK_NAME, ptr->d_name[3]);
						strcpy(alldisk->disk[alldisk->count].path, "/tmp/mnt/");
...
/*******************************************************************************
 * Function:
 *    int dm_get_elec_lock();
 * Description:
 *   get electric lock cmd = 0x0215
 * Parameters:
 *  NULL
 * Returns:
 *    0   电子锁关，允许磁盘自动挂载到电脑或设备本地
 *    1  电子锁开，不允许磁盘挂载到电脑
 *    -1  error
 *******************************************************************************/
int dm_get_elec_lock()
{
	DMCLOG_D("access dm_get_elec_lock");
	return 0;
}
/*******************************************************************************
 * Function:
 *    int dm_set_elec_lock();
 * Description:
 *   set electric lock 0x0216
 * Parameters:
 *  status:0   电子锁关，允许磁盘自动挂载到电脑或设备本地
 *            1  电子锁开，不允许磁盘挂载到电脑
 * Returns:
 *    0  success
 *    -1  error
 *******************************************************************************/
int dm_set_elec_lock(int status)
{
	DMCLOG_D("dm_set_elec_lock");
	return 0;
}
