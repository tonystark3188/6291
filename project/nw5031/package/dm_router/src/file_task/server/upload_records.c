...
int write_list_to_file(const char *path,struct record_dnode *dn)//写出数据
{
 ...
 int write_list_to_stream(FILE *record_fd,struct record_dnode *dn)//写出数据
 {
	 unsigned i = 0;
	 int n;
	 if(record_fd == NULL)
	 ...
	 for(i = 0;/*count - detected via !dn below*/;i++)
	 {
		 ...
		 DMCLOG_D("i = %d,dn->index = %d,dn->start=%lld",i,dn->index,dn->start);
		 if((n = fwrite(dn,sizeof(struct record_dnode),1,record_fd)) != 1)
		 {
 ...
 int update_record_for_index(struct record_dnode **head,off_t end,off_t cur)
 {
	 ...
	 else
	 {
		 if (node1->end == end)
		 {
			 ...
			 node1->start = cur;
			 DMCLOG_D("update dnode success, node1->start = %lld", node1->start);
			 EXIT_FUNC();
			 return 0;
		 }
		 else
