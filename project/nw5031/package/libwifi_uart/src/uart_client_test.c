...
void usage()
{
	...
	printf("cmd 8   : turn on upgrade led\n");
	printf("cmd 9   : turn off upgrade led\n");
	printf("cmd 10   : turn off sys led\n");
	...
	while((ch = getopt(argc,argv,"g:s:c"))!= -1)
	{
		switch(ch)
		{
			...
			case 's':
				...
				{
					printf("turn on upgrade led\n");
					g_uart_cmd.mode = UART_W;
					g_uart_cmd.regaddr = SOCKET_UART_SET_UPGRADE_LED;
					g_uart_cmd.data = 0;
					ret = SocketUartClientStart(&g_uart_cmd);
					if(ret == 0)
						printf("turn on upgrade led success\n");
				
				}
				else if(!strcmp(optarg,"9"))
				{
					printf("turn off upgrade led\n");
					g_uart_cmd.mode = UART_W;
					g_uart_cmd.regaddr = SOCKET_UART_SET_UPGRADE_LED;
					g_uart_cmd.data = 1;
					ret = SocketUartClientStart(&g_uart_cmd);
					if(ret == 0)
						printf("turn off upgrade led success\n");
				...
				else if(!strcmp(optarg,"10"))
				{
					...
					g_uart_cmd.mode = UART_W;
					g_uart_cmd.regaddr = SOCKET_UART_SET_OFF_SYS_LED;
					g_uart_cmd.data = 1;
					ret = SocketUartClientStart(&g_uart_cmd);
					if(ret == 0)
						printf("turn off sys led success\n");
