...
int get_scan_list_nl80211_add_network(char *ifname, char *ssid, ap_list_info_t *p_ap_list){
	...
	if(p_ap_list != NULL){
		if(ssid == NULL){
			...
			for(i = 0; i < scan_ap_list_info.count; i++){
				if(0 == strcmp(ssid, scan_ap_list_info.ap_info[i].ssid)){
					p_ap_list->count = 1;
					strcpy(p_ap_list->ap_info[0].ssid, scan_ap_list_info.ap_info[i].ssid);
					strcpy(p_ap_list->ap_info[0].mac, scan_ap_list_info.ap_info[i].mac);
					strcpy(p_ap_list->ap_info[0].encrypt, scan_ap_list_info.ap_info[i].encrypt);
					strcpy(p_ap_list->ap_info[0].tkip_aes, scan_ap_list_info.ap_info[i].tkip_aes);
					p_ap_list->ap_info[0].channel = scan_ap_list_info.ap_info[i].channel;
					p_ap_list->ap_info[0].wifi_signal = scan_ap_list_info.ap_info[i].wifi_signal;
					break;
				}
			}	
		}	
...
{
	return get_scan_list_nl80211_add_network(ifname, NULL, p_ap_list);
}
