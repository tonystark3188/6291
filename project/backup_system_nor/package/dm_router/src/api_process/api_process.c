...
/*static void release_inotify_func(void *self)
{
	...
	int ret = 0;
	int status = 0;
    release_info_t *pInfo = (release_info_t *)self;
	if(pInfo->release_flag == 1)
	{
		ret = dm_tcp_scan_notify(AIRDISK_ON_PC);
		if(ret < 0)
		{
			DMCLOG_E("scan notify error");
		}
	}	
	char *send_buf = comb_release_disk_json(status); 
    DM_MsgSend(pInfo->sock,send_buf,strlen(send_buf));
...
int create_release_task(struct conn *c)
{
    ENTER_FUNC();
    PTHREAD_T tid_release_task;
    int rslt = 0;
    release_info_t *pInfo = (release_info_t *)calloc(1,sizeof(release_info_t));
    pInfo->flags = &c->rem.flags;
    pInfo->sock = c->rem.chan.sock;
    pInfo->cmd = c->cmd;
	pInfo->release_flag = c->release_flag;
    if (0 != (rslt = PTHREAD_CREATE(&tid_release_task, NULL, (void *)release_inotify_func,pInfo)))
    {
...
{
	int ret = 0;
	ret = set_fuser_flag(c->release_flag);
	if(ret < 0){
		return ret;
	}
	c->loc.flags |= FLAG_W;
	ret = create_release_task(c);
	if(ret < 0)
	{
