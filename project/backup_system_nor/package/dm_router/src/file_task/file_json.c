...
static void del_inotify_func(void *self)
{
    ...
    if(res < 0)
    {
		DMCLOG_E("_get_info_from_dev_list");
    ...
    pInfo->flist = (file_uuid_list_t *)calloc(1,sizeof(file_uuid_list_t));
	pInfo->flist->result_cnt = 0;
	dl_list_init(&pInfo->flist->head);
    res = file_copy_compute(source_file,pInfo);
    if(res < 0)
    {
        error = ERROR_FILE_DELETE;
		pInfo->status = -1;
	...
	res = handle_backfile_special_delete_cmd(pInfo->flist,pInfo->disk_uuid);
	if(res < 0)
	{
	...
	dl_list_for_each_safe(item, n, &pInfo->flist->head, file_uuid_t, next)
    {
...
int copy_file_inotify(const char *source, const char *dest,copy_info_t *pInfo)
{
	...
	//copy handle set time function
	#if 0
	DMCLOG_D("source_stat.st_mtime = %lu", source_stat.st_mtime);
	struct utimbuf timebuf;
	timebuf.actime = source_stat.st_mtime;
	timebuf.modtime = source_stat.st_mtime;
	if(0 == utime(dest, &timebuf)){
...
int DM_FileDelListByPath(struct conn *c)
{
	...
	for(i = 0;i < mAll_disk_t.count;i++)
	{
		...
		res = handle_del_file_list_by_path_cmd(c->fileType,c->src_path,uuid);
		if(res != 0)
		{
			c->error = ERROR_GET_FILE_LIST;
			goto EXIT;
...
int DM_FileIsBackupExisted(struct conn *c)
{
	...
	DMCLOG_D("disk_uuid = %s",c->disk_uuid);
	char *path = NULL;
	res = handle_query_file_path_by_uuid(c->file_uuid,c->device_uuid,c->disk_uuid,&path);
	if(res < 0)
	{
	...
	{
		c->cfg_path = (char *)calloc(1,strlen(DOCUMENT_ROOT) + strlen(path) + strlen(CFG_PATH_NAME) + 1);
		if(c->cfg_path == NULL)
		{
			c->error = SERVER_OUT_MEMORY;;
			res = -1;
		...
		sprintf(c->cfg_path,"%s%s%s",DOCUMENT_ROOT,path,CFG_PATH_NAME);
		DMCLOG_D("c->cfg_path = %s",c->cfg_path);
		safe_free(path);
		struct stat	st;
		if(my_stat(c->cfg_path, &st) == 0)
		{
...
static void backup_inotify_func(void *self)
{
    ENTER_FUNC();
	int res = 0;
    backup_info_t *pInfo = (backup_info_t *)self;
	res = handle_backfile_insert_cmd( pInfo->file_uuid,pInfo->device_uuid,pInfo->path,pInfo->disk_uuid);
	if(res < 0)
	{
        DMCLOG_E("backup file(%s) failed", pInfo->file_uuid);
    }
...
int create_backup_insert_task(struct conn *c)
{
    ENTER_FUNC();
    PTHREAD_T tid_backup_task;
    int rslt = 0;
    backup_info_t *pInfo = (backup_info_t *)calloc(1,sizeof(backup_info_t));
	S_STRNCPY(pInfo->file_uuid,c->file_uuid,FILE_UUID_LEN);
	S_STRNCPY(pInfo->device_uuid,c->device_uuid,DEVICE_UUID_LEN);
    ...
    if (0 != (rslt = PTHREAD_CREATE(&tid_backup_task, NULL, (void *)backup_inotify_func,pInfo)))
    {
        DMCLOG_D("backup task failed!");
        rslt = -1;
        return rslt;
    }
    PTHREAD_DETACH(tid_backup_task);
    EXIT_FUNC();
    return rslt;
}
...
int Parser_FileGetBackupFile(struct conn *c)
{
	...
	*tmp = '/';
	...
	res = create_backup_insert_task(c);
	if(res < 0)
	{
	    c->error = ERROR_FILE_DELETE;
	    return res;
