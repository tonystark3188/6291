...
int dm_mcu_get_storage_dir(int *storage_dir)
{
...
int _dm_mcu_get_storage_dir(int *storage_dir)
{
	...
	ret = dm_mcu_get_storage_dir(&pc_detect);
	if(ret < 0){
		return ret;
	}

	ret = dm_get_file_storage(&pc_disable);
	if(ret < 0){
		return ret;
	}

	if(pc_disable == G_FILE_STORAGE_SET){
		*storage_dir = pc_detect;
	}else{
		*storage_dir = STORAGE_BOARD;
	}
