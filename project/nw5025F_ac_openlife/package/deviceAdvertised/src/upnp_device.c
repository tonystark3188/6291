...
static int aNewOne=1;
static char last_sid[1024];
...
//	ENTER();
	//jh strcpy(last_sid, sr_event->Sid);
	...
	for(i=0; i<eventVarCount; i++) {
		if(eventvar_values[i])
		{free(eventvar_values[i]);  eventvar_values[i] = NULL;}
	}
	if(eventvar_names)
	{free(eventvar_names); eventvar_names = NULL;}
	if(eventvar_values)
	{free(eventvar_values); eventvar_values = NULL;}
					  ...
					  *ar_event)
{
	...
	struct action *event_action; // printf("enter %s\n",__FUNCTION__);
...
int upnp_device_init(struct device *device_def, char *ip_address,char *socketport)
{
	int rc;
	int result = -1;
	short int port = 49211;
	struct service *srv;
	...
	if (UPNP_E_SUCCESS != rc) {
       ...
       if( NULL == ip_address ) 
       {//printf("NULL == ip_address\n");
          ip_address = UpnpGetServerIpAddress();
       }

       	buf = upnp_get_device_desc(device_def,ip_address,socketport);
	...
	rc = UpnpSendAdvertisement(device_handle, g_duration);
	if (UPNP_E_SUCCESS != rc) {
