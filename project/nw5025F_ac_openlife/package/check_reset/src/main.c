...
#ifndef SUPPORT_WPS_KEY
#define SUPPORT_WPS_KEY 1
#define WPS_TIMEOUT 90//S
#define WPSRUINGNODE "/tmp/WPSing"
#endif
...
int main() 
{
	...
	while(1){	
		usleep(800000);
#if SUPPORT_WPS_KEY//JJJHHH
        wps_fp = fopen(WPSRUINGNODE, "r");
		if(wps_fp)
		{   
		    gettimeofday(&CurrentTime,NULL);
			if(0 == wps_starttime)
				wps_starttime = CurrentTime.tv_sec;
			else if((CurrentTime.tv_sec - wps_starttime)>=WPS_TIMEOUT)//timeout
			{
			    fclose(wps_fp);
			    wps_fp = NULL;
...
#if SUPPORT_WPS_KEY//JJJHHH
            if(0 == wps_starttime)
			    system("/sbin/startWPS");
#else
            system("sh /sbin/2g_5g_switch.sh &");
#endif
		}
		else
			printf("reset_status=%d",reset_status);
