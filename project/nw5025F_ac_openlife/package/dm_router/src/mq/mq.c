...
int release_mq(MQObj *p_mq)
{
    ...
    if(!p_mq->is_valid)
    {
        log_warning("%s is not valid!", p_mq->mq_name);
        return EINVAL_OBJ;
    }
    p_mq->is_valid = 0;

	log_trace("start(%s)", p_mq->mq_name);
...
int unregister_mq(MSG_TYPE msg_type)
{
    ...
    if(index > MAX_MQ_INDEX)
    {
        log_warning("mq_index(%d) too big!", index);
        return EOUTBOUND;
    }
    ...
    if(g_mqs[index] == NULL)
    {
        log_warning("mq_index(%d) has unregister!", index);
        return -2;
    }
#endif
...
static MsgObj * _recv_msg(MQObj *p_mq, uint8_t no_wait)
{
	...
	//log_trace("_recv_msg start(%s)", p_mq->mq_name);
	mq_lock(&p_mq->mq_lock);

    if(no_wait == 0)
    {
    	while(p_mq->msg_nums == 0)
    	{
...
static int _send_msg_sync(MQObj *p_mq, MsgObj *p_msg)
{
	...
	if(p_msg->m_syn == NULL) 
	{
		log_warning("NULL pointer 2!");
		return ENULL_POINT;
	}

	_send_msg(p_mq, p_msg);
	log_trace("send msg finish, wait response!");
...
static int _send_message(Message *p_msg, uint8_t is_syn)
{
	...
	if(p_msg->msg_header.m_type & 0x01) // msg request
	{
		...
		if(p_mq == NULL)
		{
			log_notice("find mq by msg_type(%d) failed!", p_msg->msg_header.m_type);
			return -2;
		}	
