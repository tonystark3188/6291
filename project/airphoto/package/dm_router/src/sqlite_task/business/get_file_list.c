...
{   
    ...
    if(len > 0)
    {
    	...
    	dl_list_for_each(p_file_info, &(get_db_opr_obj(msg).data.file_list.head), file_info_t, next)
    	{
			...
			JSON_ADD_OBJECT(file_info, "path",JSON_NEW_OBJECT(p_file_info->path,string));
...
{   
	...
	char bIsRegularFile = get_db_opr_obj(msg).data.file_data.dir_type;
	if(bIsRegularFile != 2)
	{
...
{   
	...
	unsigned long long llSize = statbuf.st_size;
	unsigned long long lAccessTime = statbuf.st_atime;
    // add end.
	//file_info_t file_info;
    ...
    if(bIsRegularFile)
    {
        get_db_opr_obj(msg).data.insert_data.file_info.file_size= llSize;
    }
    else
    {
        get_db_opr_obj(msg).data.insert_data.file_info.file_size = 0;
    }
	get_db_opr_obj(msg).data.insert_data.file_info.dir_type = bIsRegularFile;//the file is dir
	get_db_opr_obj(msg).data.insert_data.file_info.create_time = lAccessTime;
...
int dm_getTypeInfo(int file_type,struct file_list *file_list_t,char *uuid)
{
	unsigned nfiles = 0;
	int res = 0;
	res = handle_get_file_list_count_cmd(file_list_t->file_type,&nfiles,uuid);
	if(res != 0)
	{
...
/*!
 ...
 @param char session[32];
		...
		char username[128];
		char password[128];
 @result INT(返回值大于等于0表示成功，否则表示失败)
*/
...
int bind_devuuid_to_diskuuid(_In_ const char *device_uuid,_In_ const char *device_name,_In_ const char *disk_uuid)
{
	...
	S_STRNCPY(get_db_opr_obj(msg).data.device_data.device_name, device_name, MAX_USER_DEV_NAME_LEN);
    S_STRNCPY(get_db_opr_obj(msg).data.device_data.device_uuid, device_uuid, MAX_MAC_ADDR_STR_LEN);
...
{   
	...
	if(get_db_opr_obj(msg).data.backup_file_data.file_status == 1)//文件还在备份中
	{
		free_sync_message(&msg);
		return 1;
	}
...
{   
	...
	int ret = 0;
    ENTER_FUNC();
	if(path == NULL || file_uuid == NULL || device_uuid == NULL)
	{
		log_error("para is null");
        return EMESSAGE_NEW;
	}
	...
	snprintf(get_msg_bk_info(msg).file_uuid, MAX_USER_DEV_NAME_LEN, "%s", file_uuid);
	snprintf(get_msg_bk_info(msg).name, BACKUP_PATH_SIZE, "%s", bb_basename(path));
	snprintf(get_msg_bk_info(msg).path, 1024, "%s", path + strlen(DOCUMENT_ROOT));
    get_msg_bk_info(msg).size = llSize;
    //snprintf(get_msg_bk_info(msg).device_name, MAX_USER_DEV_NAME_LEN, "%s", device_name);
	snprintf(get_msg_bk_info(msg).device_uuid, MAX_USER_DEV_NAME_LEN, "%s", device_uuid);
    get_msg_bk_info(msg).backup_time = lAccessTime;
	get_msg_bk_info(msg).file_type = db_get_mime_type(path,strlen(path));
...
int db_backup_file_rm(const char *path,const char *disk_uuid)
{
	...
	if(ret == 0)
	{
		handle_backfile_delete_cmd(file_uuid,disk_uuid);
	}
    return 0;
}
