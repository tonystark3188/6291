...
static error_t load_user_insert_cmd(char *sql, user_info_t *puser)
{
	...
	n = snprintf(sql, SQL_CMD_WRITE_BUF_SIZE, "INSERT INTO %s(SESSION,USER_NAME,PASSWORD,IP,DEVICE_UUID,"\
		...
		puser->device_uuid, puser->device_name);
...
static void load_user_info(user_info_t *puser, char **FieldValue)
{
	...
	if(FieldValue[4] != NULL)
		S_STRNCPY(puser->device_uuid, FieldValue[4], 32);
	DMCLOG_D("puser->device_uuid = %s",puser->device_uuid);

	if(FieldValue[5] != NULL)
		S_STRNCPY(puser->device_name, FieldValue[5], 32);
	...
	if(FieldValue[2] != NULL)
		S_STRNCPY(puser->password,  FieldValue[2], MAX_USER_PASSWD_LEN);

	if(FieldValue[3] != NULL)
		puser->is_valid = atoi(FieldValue[3]);
	
    if(FieldValue[4] != NULL)
	    puser->hd_used_size    = strtoull(FieldValue[4], NULL, 10);
	
    if(FieldValue[5] != NULL)
	    puser->hd_recycle_size = strtoull(FieldValue[5], NULL, 10);
	
    if(FieldValue[6] != NULL)
	    puser->hd_video_size   = strtoull(FieldValue[6], NULL, 10);
	
    if(FieldValue[7] != NULL)
	    puser->hd_audio_size   = strtoull(FieldValue[7], NULL, 10);
	
    if(FieldValue[8] != NULL)
	    puser->hd_photo_size   = strtoull(FieldValue[8], NULL, 10);
	#endif
}
