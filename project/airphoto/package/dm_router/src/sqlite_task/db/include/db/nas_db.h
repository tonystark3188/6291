...
typedef struct
{
	...
	uint32_t hdisk_id;	    //which hdisk the file belongs to.
}scan_data_t;
...
typedef struct
{
	...
	//uint32_t user_id;     //which user the copied file belongs to
	//uint32_t hdisk_id;    //which hdisk the copied file belongs to
}copy_data_t;
...
typedef struct
{
	...
	uint32_t dir_id;			  //if we are querying the file list in a certain directory, tell the directory id.
...
typedef struct DB_OprObj
{
	union
	{
		...
		trash_data_t   		trash_data;      // using for trash record
		recycle_data_t 	    recycle_data;    //using for recycle record
		...
		dir_list_t         dir_list;       //using for file list query
 		set_share_t         share_data;      //using for setting share state
