...
typedef struct
{
    ...
    uint32_t hdisk_id;   //the id of hdisk the file belongs to.
} trash_data_t;
...
typedef error_t (*generic_delete_item_fn)(sqlite3 *database, uint8_t cmd, void *target, delete_callback callback);
typedef error_t (*special_delete_item_fn)(sqlite3 *database, void *target, update_info_t *pupdate_info,
