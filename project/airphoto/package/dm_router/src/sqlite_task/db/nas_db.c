...
error_t db_insert_backup_file(sqlite3 *database, backup_file_info_t *puv)
{
	...
	ui.action = UPDATE_ADD;
	ui.hdisk_id = 1;
	ui.user_id =  puv->owner;	
	ui.total_update_size = puv->size;
...
#if 0
	log_debug("start updating user table");
	
	if((errcode = g_user_table.ops.passive_update(database, &ui)) != RET_SUCCESS)
	{
		log_warning("error when update user table(user id = %u) after add new backup file(id = %u),"\
			"errcode = 0x%X", ui.user_id, puv->id, errcode);
		return errcode;
	}
	log_debug("update user table ok");
	
	log_debug("start updating hdisk table");
	if((errcode = g_hdisk_table.ops.passive_update(database, &ui)) != RET_SUCCESS)
	{
		log_warning("error when update hdisk table(id = %u) after add new backup file(id=%u),"\
			"errcode = 0x%X", ui.hdisk_id, puv->id, errcode);
		return errcode;
	}
	log_debug("update hdisk table ok");
#endif
	log_debug("exit ok......");
...
*return: RET_SUCCESS if ok
*/
error_t db_delete_backup_file(sqlite3 *database, delete_data_t *pdata)
{
	update_info_t ui;
	error_t errcode = RET_SUCCESS;
	...
	ui.user_id = pdata->user_id;
	//ui.hdisk_id = pdata->hdisk_id;
	ui.action = UPDATE_DEL_NORM;

	log_debug("start deleting backup file in backup file table");
	if((errcode = g_backup_file_table.ops.special_delete(database, &pdata->target_id, 
			&ui, NULL)) != RET_SUCCESS)
	{
		if(errcode == EDB_RECORD_NOT_EXIST)
		{
		...
		log_warning("error when delete backup file(id=%u),errcode=0x%X",
			pdata->target_id, errcode);
		return errcode;
	}
	...
	if((errcode = g_user_table.ops.passive_update(database, &ui)) != RET_SUCCESS)
	{
		log_warning("error when update user table(user id = %u) after delete backup file(id = %u),"\
			"errcode = 0x%X", ui.user_id, pdata->target_id, errcode);
		return errcode;
	}
	log_debug("update user table ok");
	...
	if((errcode = g_hdisk_table.ops.passive_update(database, &ui)) != RET_SUCCESS)
	{
		log_warning("error when update hdisk table(id = %u) after delete backup file(id=%u),"\
			"errcode = 0x%X", ui.hdisk_id, pdata->target_id, errcode);
		return errcode;
	}
	log_debug("update hdisk table ok");*/
...
error_t db_query(sqlite3 *database, QueryApproach approach, void *data)
{
	...
	switch(approach)
	{
		...
		case QUERY_FILE_BY_NAME:
		{
			...
			log_debug("start querying file info");
...
static error_t db_init(bool hdisk_formated)
{
	...
	if(hdisk_formated)
	{
		...
		{
			DMCLOG_D("create db backup file error");
			return errcode;
		}
		#endif
	}
...
void register_db_table_ops(void)
{
    ...
    snprintf(DATABASE, 128, "%s/..airdisk.db", get_sys_dm_db_path());
...
void register_disk_table_ops(struct disk_node *disk_info)
{
	...
	snprintf(disk_info->g_database, 128, "%s/..airdisk.db", disk_info->path);
