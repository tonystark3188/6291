...
{
	int ret = -1;
	ret = DM_UdpSend(p_client_info->client_fd, p_client_info->retstr, sizeof(p_client_info->retstr),&p_client_info->clientAddr);
	if (ret != 0)
	{
		DMCLOG_D("DM send failed with %d,acc_fd = %d",ret,p_client_info->client_fd);
		return -1;
	}
	return 0;
}
