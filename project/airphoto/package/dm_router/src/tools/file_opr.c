...
#if 0
int dm_getFileList(const char *path, unsigned *nfiles_p,struct file_dnode **dn,unsigned long offset,unsigned long length,JObj *response_para_json)
{
	struct file_dnode *cur;
	struct dirent *entry;
	DIR *dir;
...
int create_mark_file(char *path,char *uuid)
{
	...
	sprintf(uuid_path,"%s/%s",path,DM_DISK_UUID);
	FILE *fd;
...
int read_mark_file(char *path,char *uuid)
{
	...
	fclose(fd);
	DMCLOG_D("uuid = %s,length = %d",uuid,sizeof(uuid));
	return 0;
}
