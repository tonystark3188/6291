...
static MsgRet DM_WaitDataAvailable(SINT32 iFd, UINT32 nTimeOut)
{
    ...
    {
    	DMCLOG_E("recv timeout,socket = %d", iFd);
        return MSGRET_TIMED_OUT;
    }
}
...
SINT32 DM_InetClientInit(SINT32 iDomain, SINT32 iPort, SINT32 iType,SINT8 *ipaddr)
{
    ...
    if (iRc != 0)
    {
    	DMCLOG_D("iFd = %d,errno = %d",iFd,errno);
        close(iFd);
        return -1;
    }
    else
...
SINT32 DM_ServerAcceptClient(SINT32 iListenFd)
{
	...
	DMCLOG_D("iAcceptFd = %d",iAcceptFd);
    return iAcceptFd;
}
...
static MsgRet DM_WaitSendAvailable(SINT32 iFd, UINT32 nTimeOut_ms)
{
    struct timeval stTv;
    fd_set fsWriteFds;
    SINT32 iRc;
    ...
    FD_ZERO(&fsWriteFds);
    FD_SET(iFd, &fsWriteFds);

	if(nTimeOut_ms > 0)
	{
		stTv.tv_sec = nTimeOut_ms / MSECS_IN_SEC;
		stTv.tv_usec = (nTimeOut_ms % MSECS_IN_SEC) * USECS_IN_MSEC;
		iRc = select(iFd+1, NULL, &fsWriteFds, NULL, &stTv);
	}else{
		iRc = select(iFd+1, NULL, &fsWriteFds, NULL, NULL);
	}
   
    if ((iRc == 1) && (FD_ISSET(iFd, &fsWriteFds)))
    {
        return MSGRET_SUCCESS;
    }
    else
    {
    	DMCLOG_E("send timeout,socket = %d", iFd);
        return MSGRET_TIMED_OUT;
    }
}
...
SINT32 DM_MsgSend(SINT32 iFd, const SINT8 *pBuf, UINT32 nLen)
{
	...
	temp = (MsgHeader *)realloc(pstMsg, sizeof(MsgHeader) + nLen);
	if (temp == NULL)
	{
	...
	pstMsg = (MsgHeader *)temp;
	memcpy((SINT8 *)(pstMsg+1), pBuf, nLen);
	
	nTotalLen = sizeof(MsgHeader) + nLen;
	//DMCLOG_D("total send:%d\n", nTotalLen);
	SINT32 bytesent = 0;
	
	while(1) {
		if (DM_WaitSendAvailable(iFd, 3000) < 0) {
			free(pstMsg);
			return MSGRET_TIMED_OUT;			
		}
		iRc = write(iFd, (SINT8 *)pstMsg+bytesent, nTotalLen-bytesent);
...
SINT32 DM_MsgReceive(SINT32 iFd, SINT8 **pBuf, UINT32 *pnTimeOut)
{
        ...
        ((iRc == -1) && (errno == 131)))  /* new 2.6.21 kernel seems to give us this before rc==0 */
    {
        /* broken connection */
        free(pstMsg);
		DMCLOG_E("tcp read failed,fd = %d,reson: %s", iFd, iRc == 0?"peer close the connection":strerror(errno) );
        return MSGRET_DISCONNECTED;
    }
    else if (iRc < 0 || iRc != sizeof(MsgHeader))
    {
        DMCLOG_E("bad read, rc=%d errno=%d reason:%s",
               iRc, errno, strerror(errno));
        free(pstMsg);
        return MSGRET_INTERNAL_ERROR;
    }
	...
	//DMCLOG_D("nTotalRemaining:%d", nTotalRemaining);
    if (nTotalRemaining > 0)
    {
        ...
        if (pInBuf == NULL)
        {
            DMCLOG_E("malloc %d bytes failed",
                   nTotalRemaining);
            free(pstMsg);
            //free(pInBuf);
            return MSGRET_RESOURCE_EXCEEDED;
        ...
        while (nTotalReadSoFar < nTotalRemaining)
        {
            ...
            if (iRc <= 0)
            {
                DMCLOG_D("bad data read, fd = %d,rc=%d errno=%d readSoFar=%d remaining=%d reason:%s", 
                    iFd, iRc, errno, nTotalReadSoFar, nTotalRemaining, iRc == 0?"peer close the connection":strerror(errno));
                free(pstMsg);
                free(pInBuf-nTotalReadSoFar);
                return MSGRET_INTERNAL_ERROR;
            }
...
SINT32 DM_UdpSend(SINT32 iFd, const SINT8 *pBuf, UINT32 nLen,struct sockaddr_in *addr )
{
    ...
    memcpy((SINT8 *)(pstMsg + 1), pBuf, nLen);
    nTotalLen = sizeof(MsgHeader) + nLen;
	DMCLOG_D("nTotalLen = %d, len = %d, pstMsg = %s, pstMsg+1 = %s",nTotalLen , len, pstMsg, (pstMsg+1) );
...
int DM_InotifyReceive(int iFd,UINT32 *pnTimeOut)
{
	...
	DMCLOG_D("length = %d,buffer = %s",length,buffer);
    if(length < 0) 
	{
    ...
    {  
        ...
        if ( event->len )   
        {  
        	DMCLOG_D("event->len = %d",event->len);
            ...
            {  
                if ( event->mask & IN_ISDIR )  
               	...
               	{
					DMCLOG_D("IN_MODIFY=%08X; IN_CREATE=%08X; IN_DELETE=%08X; IN_ACCESS=%08X; IN_OPEN=%08X; IN_CLOSE=%08X", IN_MODIFY, IN_CREATE, IN_DELETE, IN_ACCESS, IN_OPEN, IN_CLOSE);
					DMCLOG_D( "The file %s was modified.Event mask: %08X\n", event->name, event->mask); 
        ...
        {  
			...
			DMCLOG_D("specified by add watch function");
		}
		DMCLOG_D("access inotify");
