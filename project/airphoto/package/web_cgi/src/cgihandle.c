...
int setssid(xmlNodePtr tag, char *retstr)  //network restart
{
	...
	if(strlen(name))
	{
		sprintf(str_sp,"wireless.@wifi-iface[0].ssid=%s",name);
...
//		system(str_sp);
	...
	{
		sprintf(str_sp,"wireless.@wifi-iface[0].encryption=%s",encrypt_config);
		uci_set_option_value(str_sp);
		memset(str_sp,0,64);
...
//		system(str_sp);
	...
	else if(!strcmp(encrypt_config,"psk") || !strcmp(encrypt_config,"psk2") || !strcmp(encrypt_config,"mixed-psk"))
	{
		...
		{
			sprintf(str_sp,"wireless.@wifi-iface[0].encryption=%s+%s",encrypt_config,tkip_aes_config);
			uci_set_option_value(str_sp);
			memset(str_sp,0,64);
