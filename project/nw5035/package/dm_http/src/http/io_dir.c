 ...
 */
int
put_dir(struct conn *c, const char *path)
{
...
#ifdef _PPCLIB_H_
int
ppc_put_dir(struct conn *c, const char *path)
{
	char		buf[FILENAME_MAX];
	const char	*s, *p;
	#ifdef _PPCLIB_H_
...
read_dir(struct stream *stream, void *buf, size_t len)
{
	...
	do {
		...
		#ifdef _PPCLIB_H_
		if ((dp = ppc_readdir(stream->chan.dir.dirp, c->token)) == NULL) 
		#else
...
close_dir(struct stream *stream)
{
	...
	#ifdef _PPCLIB_H_
	(void) ppc_closedir(stream->chan.dir.dirp, stream->conn->token);
	#else
