...
/*******************************************************************************
 * Function:
 *   SINT32 DM_WaitDataAvailable_t(SINT32 iFd, UINT32 nTimeOut)
 * Description:
 *    Wait for available data until timeout
 * Parameters:
 *    iFd      (IN) socket file descriptor
 *    nTimeOut (IN) Time for timeout
 * Returns:
 *    0:wait sending;-1,error,1:wait receiving
 *******************************************************************************/
SINT32 DM_WaitDataAvailable_t(SINT32 iFd, UINT32 nTimeOut)
{
    struct timeval stTv;
...
SINT32 DM_InetServerInit(SINT32 iDomain, SINT32 iPort, SINT32 iType, SINT32 iBackLog)
{
    ...
    if (iType == SOCK_STREAM)
    {
        ...
        {
            DMCLOG_D("listen errno=%d port=%d fd=%d reason:%s",
                   errno, iPort, iFd, strerror(errno));
            close(iFd);
            return MSG_INVALID_FD;
        }
...
//    system(szTmp);
    ...
    if (iRc != 0)
    {
        DMCLOG_D("bind to %s failed, rc=%d errno=%d reason:%s",
    ...
    if (iRc != 0)
    {
        DMCLOG_D( "listen to %s failed, rc=%d errno=%d reason:%s",
               stServerAddr.sun_path, iRc, errno, strerror(errno));
        close(iFd);
        return -1;
    }
    
    DMCLOG_D("%s msg socket opened and ready (fd=%d)",
...
SINT32 DM_DomainClientInit(const SINT8 *pServerName)
{
    ...
    if (NULL == pServerName)
    {
        DMCLOG_D( "Server APP's name is null!");
        return -1;
    }
    ...
    if (iRc != 0)
    {
        DMCLOG_D("connect to "APP_MESSAGE_PATH"%s failed, rc=%d errno=%d reason:%s",
    ...
    {
//        DMCLOG_D("commFd=%d connected to "APP_MESSAGE_PATH"%s",
//               iFd, pServerName);
    }
...
SINT32 DM_ServerAcceptGetIp(SINT32 iListenFd,char *clientIp)
{
    ...
    DMCLOG_D("[%d] connected from %s:%d", iAcceptFd,
...
SINT32 DM_MsgSend(SINT32 iFd, const SINT8 *pBuf, UINT32 nLen)
{
    ...
    if (pstMsg == NULL)
    {
        DMCLOG_D("realloc to %ld bytes failed",
    ...
    if (iRc < 0)
    {
        if (errno == EPIPE)
        {
             ...
             */
            DMCLOG_D("got EPIPE, dest app is dead, errno:%d reason:%s",
                   errno, strerror(errno));
            free(pstMsg);
        ...
        {
            DMCLOG_D("write failed, errno=%d reason:%s",
                   errno, strerror(errno));
            free(pstMsg);
            return MSGRET_INTERNAL_ERROR;
        }
...
SINT32 DM_FileSend(SINT32 iFd, const SINT8 *pBuf, UINT32 nLen)
{
    UINT32 nTotalLen;
    SINT32 iRc;
    MsgRet enRet = MSGRET_SUCCESS;
    ...
    nTotalLen = sizeof(MsgHeader) + nLen;
	DMCLOG_D("nTotalLen = %d",nTotalLen);
    iRc = write(iFd, (SINT8 *)pstMsg, nTotalLen);
    if (iRc < 0)
    {
        if (errno == EPIPE)
...
SINT32 DM_MsgReceive(SINT32 iFd, SINT8 **pBuf, UINT32 *pnTimeOut)
{
    ...
    {
        DMCLOG_D("bad read, rc=%d errno=%d reason:%s",iRc, errno, strerror(errno));
        free(pstMsg);
        return MSGRET_INTERNAL_ERROR;
    }
    ...
    if (nTotalRemaining > 0)
    {
        ...
        while (nTotalReadSoFar < pstMsg->dataLength)
        {
            ...
            if (pnTimeOut)
            {
                ...
                {
                    free(pstMsg);
                    free(pInBuf - nTotalReadSoFar);//free current address
                    return enRet;
                }
            }
            iRc = read(iFd, pInBuf, nTotalRemaining);
            //printf("iRc = %d,pInBuf = %s\n",iRc,pInBuf);
            if (iRc <= 0)
            {
...
/*******************************************************************************
 * Function:
 *    SINT32 DM_MsgReceive_Try(SINT32 iFd, SINT8 **pBuf, UINT32 *pnTimeOut)
 * Description:
 *    Use this function,server recieve message from client,
 *    or client recieve response message from server
 * Parameters:
...
SINT32 DM_MsgReceive_Try(SINT32 iFd, SINT8 **pBuf, UINT32 *pnTimeOut)
{
    ...
    if (nTotalRemaining > 0)
    {
        ...
        while (nTotalReadSoFar < pstMsg->dataLength && tryTimes < 100)
        {
            /*DM_LOG(DM_MSG_LOG_FLAG, DM_MSG_LOG_INFO, "reading segment: soFar=%d total=%d",
             nTotalReadSoFar, nTotalRemaining);*/
			if (pnTimeOut)
			...
			if (iRc <= 0)
            {
                /*DM_LOG(DM_MSG_LOG_FLAG, DM_MSG_LOG_ERR, "bad data read, rc=%d errno=%d readSoFar=%d remaining=%d reason:%s",
                 iRc, errno, nTotalReadSoFar, nTotalRemaining, strerror(errno));*/
                DMCLOG_D("errno:%d reason:%s",errno, strerror(errno));
				if (errno == EAGAIN){
					printf("try again");
					tryTimes++;
					usleep(100);
	...
	DMCLOG_D("nTotalReadSoFar: %d", nTotalReadSoFar);
    *pBuf = pInBuf - nTotalReadSoFar;
	free(pstMsg);
    return nTotalReadSoFar;
}
...
SINT32 DM_UdpReceive(SINT32 iFd, SINT8 **pBuf, UINT32 *pnTimeOut,struct sockaddr_in *addr)
{
    ...
    memcpy(pInBuf,pstMsg+1,pstMsg->dataLength);
