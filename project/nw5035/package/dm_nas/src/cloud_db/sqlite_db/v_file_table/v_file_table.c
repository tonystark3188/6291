...
static error_t load_v_file_insert_cmd(sqlite3 *database,v_file_info_t *pfile,char *bucket_name)
{
	...
	if(pfile->isDir == 1)
	{
		snprintf(sql, SQL_CMD_WRITE_BUF_SIZE, "INSERT INTO %s(FILE_NAME,FILE_PATH,REAL_FILE_ID,PARENT_ID,IS_DIR,TYPE)"\
			"VALUES('%s','%s','%d','%d','%d','%d')",bucket_name,
			name, path_escape, pfile->real_file_id,pfile->parent_id,pfile->isDir,pfile->type);
	}else{
		snprintf(sql, SQL_CMD_WRITE_BUF_SIZE, "INSERT INTO %s(FILE_NAME,REAL_FILE_ID,PARENT_ID,IS_DIR,TYPE)"\
			"VALUES('%s','%d','%d','%d','%d')",bucket_name,
			name, pfile->real_file_id,pfile->parent_id,pfile->isDir,pfile->type);
	}
	safe_free(path_escape);
									 ...
									 v_file_list_t *file_list,char *bucket_name)
{
	...
	sprintf(sql, "SELECT \
		...
		%s.MODIFY_TIME,\
		%s.UUID\
	FROM %s left join %s on %s.REAL_FILE_ID=%s.FILE_ID WHERE %s %s %s", bucket_name,bucket_name,bucket_name,bucket_name,FILE_TABLE_NAME,FILE_TABLE_NAME,FILE_TABLE_NAME,\
	bucket_name,FILE_TABLE_NAME,bucket_name,FILE_TABLE_NAME,\
		file_type_buf, parent_limit_buf,tail);
									 ...
									 char *des_path,char *bucket_name)
{
	...
	if(isfolder == 1)
	{
		sprintf(sql,"UPDATE %s SET FILE_NAME='%s',FILE_PATH='%s', PARENT_ID='%d' WHERE FILE_ID='%d'",bucket_name, name,path_escape,parent_id, id);
	}	
	else
	{
		int type = db_get_mime_type(des_path,strlen(des_path));
		sprintf(sql,"UPDATE %s SET FILE_NAME='%s',PARENT_ID='%d',TYPE='%d' WHERE FILE_ID='%d'", bucket_name,name,parent_id,type,id);
	}
...
static error_t v_file_table_update_name(sqlite3 *database, int id, char *name,char *bucket_name)
{
	ENTER_FUNC();
	char sql[SQL_CMD_WRITE_BUF_SIZE] = {0};
	error_t errcode = RET_SUCCESS;
	...
	sprintf(sql,"UPDATE %s SET FILE_NAME='%s' WHERE FILE_ID=%d",bucket_name,name,id);
...
static error_t v_file_table_rename_by_id(sqlite3 *database, int id,int isfolder,
									 char *des_path,char *bucket_name)
{
	ENTER_FUNC();
	char sql[SQL_CMD_WRITE_BUF_SIZE] = {0};
	...
	if(isfolder == 1)
	{
		sprintf(sql,"UPDATE %s SET FILE_NAME='%s',FILE_PATH='%s' WHERE FILE_ID=%d", bucket_name, bb_basename(path_escape),path_escape,id);
	}	
	else
	{
		sprintf(sql,"UPDATE %s SET FILE_NAME='%s' WHERE FILE_ID=%d", bucket_name, bb_basename(path_escape),id);
	}
...
error_t v_file_table_update(sqlite3 *database,void *data)
{
	...
	{
		errcode = load_v_file_cmd(database,pfile,pfile_update->bucket_name);
		if(errcode != RET_SUCCESS)
		{
		...
		return load_file_update_cmd(database,pfile);
...
error_t v_file_table_query(sqlite3 *database,void *data)
{
	...
	{
		 return load_uuid_exist_cmd(database,v_file_info->uuid);
	}else if(pfile_query->cmd == V_FILE_TABLE_QUERY_LIST_BY_UUID)
	{
		...
		dl_list_for_each(p_file_info, &(pfile_query->v_file_list.head), v_file_info_t, next)
		{
			...
			if((errcode = load_uuid_exist_cmd(database,p_file_info->uuid)) != RET_SUCCESS)
		    {
