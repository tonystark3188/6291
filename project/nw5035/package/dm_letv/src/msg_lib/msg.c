 ...
 *    0:success;others,error
 *******************************************************************************/
MsgRet DM_WaitDataAvailable(SINT32 iFd, UINT32 nTimeOut)
{
    struct timeval stTv;
    fd_set fsReadFds;
    SINT32 iRc;
...
SINT32 DM_InetServerInit(SINT32 iDomain, SINT32 iPort, SINT32 iType, SINT32 iBackLog)
{
    ...
    {
        p_debug( "socket errno=%d port=%d reason:%s", errno, iPort, strerror(errno));
    ...
    {
        p_debug("setsockopt errno=%d port=%d fd=%d reason:%s", 
    ...
    if (iDomain == AF_INET)
    {
        ...
        {
            p_debug( "bind errno=%d port=%d fd=%d reason:%s",
                   errno, iPort, iFd, strerror(errno));
            close(iFd);
            return MSG_INVALID_FD;
        }
...
SINT32 DM_InetClientInit(SINT32 iDomain, SINT32 iPort, SINT32 iType,SINT8 *ipaddr)
{
    ...
    {
        DM_LOG(DM_MSG_LOG_FLAG, DM_MSG_LOG_INFO, "commFd=%d connected to "APP_MESSAGE_PATH,
               iFd);
    }
...
SINT32 DM_ServerAcceptGetIp(SINT32 iListenFd,char *clientIp)
{
    ...
    {
        p_debug("accept IPC connection failed. errno=%d reason:%s", errno, strerror(errno));
        return -1;
    }
	getpeername(iListenFd, (struct sockaddr *)&stClientAddr, &nSockAddrSize);
    if(iAcceptFd < 0) {
        p_debug("[%d] %s", errno, strerror(errno));
        return -1;
    }
    p_debug("[%d] connected from %s:%d", iAcceptFd,inet_ntoa(stClientAddr.sin_addr), ntohs(stClientAddr.sin_port));
	if(clientIp == NULL)
		return -1;
...
SINT32 DM_MsgSend(SINT32 iFd, const SINT8 *pBuf, UINT32 nLen)
{
	...
	if (pstMsg == NULL)
	{
		p_debug("realloc to %ld bytes failed", sizeof(MsgHeader) + nLen);
		free(pstMsg);
		return MSGRET_RESOURCE_EXCEEDED;
	}
...
SINT32 DM_UdpSend(SINT32 iFd, const SINT8 *pBuf, UINT32 nLen,struct sockaddr_in *addr )
{
    ...
    if (iRc < 0)
    {
        p_debug("write failed, errno=%d reason:%s",errno, strerror(errno));
        free(pstMsg);
        enRet = MSGRET_INTERNAL_ERROR;
        return enRet;
    }
...
int DM_InotifyReceive(int iFd,UINT32 *pnTimeOut)
{
	...
	p_debug("length = %d,buffer = %s",length,buffer);
    if(length < 0) 
	{
    ...
    {  
        ...
        if ( event->len )   
        {  
        	p_debug("event->len = %d",event->len);
            if ( event->mask & (IN_MODIFY | IN_CREATE | IN_DELETE | IN_ACCESS | IN_OPEN) )   
            {  
                if ( event->mask & IN_ISDIR )  
                    p_debug( "The directory %s was modified.", event->name );  
                else  
               	{
					p_debug("IN_MODIFY=%08X; IN_CREATE=%08X; IN_DELETE=%08X; IN_ACCESS=%08X; IN_OPEN=%08X; IN_CLOSE=%08X", IN_MODIFY, IN_CREATE, IN_DELETE, IN_ACCESS, IN_OPEN, IN_CLOSE);
					p_debug( "The file %s was modified.Event mask: %08X\n", event->name, event->mask); 
        ...
        {  
			...
			p_debug("specified by add watch function");
		}
		p_debug("access inotify");
