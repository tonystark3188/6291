...
#	sync
	...
	v "Upgrade completed"
	if [ -f /tmp/dont_reboot ]; then
		#echo none > /sys/class/leds/longsys\:wifi\:led/trigger
		echo timer > /sys/class/leds/longsys\:green\:led/trigger
		echo 100 > /sys/class/leds/longsys\:green\:led/delay_on
		echo 100 > /sys/class/leds/longsys\:green\:led/delay_off
		echo timer > /sys/class/leds/longsys\:blue\:led/trigger
		echo 100 > /sys/class/leds/longsys\:blue\:led/delay_on
		echo 100 > /sys/class/leds/longsys\:blue\:led/delay_off
		exit 0
	fi
	[ -n "$DELAY" ] && sleep "$DELAY"
	ask_bool 1 "Reboot" && {
