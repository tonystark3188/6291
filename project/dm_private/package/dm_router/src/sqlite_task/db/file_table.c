...
//static error_t delete_directory(sqlite3 *database, delete_data_t *pdel_data, file_info_t *pfi,update_info_t *update_info,delete_callback clean_up_fn);
...
//    log_debug("start");
	...
	sprintf(sql, "SELECT * FROM %s where NAME = '%s' AND PARENT_ID = %u",
		FILE_TABLE_NAME, name,parent_id);
	if((errcode = sqlite3_exec_busy_wait(database, sql, file_query_callback, pfi))
			!= RET_SUCCESS)
	{
        log_debug("exit error");
...
static error_t load_file_insert_cmd(char *sql,file_info_t *pfi)
{
	...
	if(pfi->isFolder)
	{
        ...
        "VALUES(%u,'%s','%s',%u,%d,%lu,%u,%d,%d,'%s','%s',%u,%u);", FILE_TABLE_NAME,
         pfi->index, pfi->name_escape,pfi->path,pfi->parent_id, pfi->file_type, pfi->file_size,
         pfi->create_time,pfi->media_info_index,pfi->isFolder,pfi->file_uuid,pfi->mime_type,pfi->modify_time,pfi->access_time);
	}else{
		n = snprintf(sql, SQL_CMD_WRITE_BUF_SIZE, "INSERT INTO %s(ID,NAME,PATH,PARENT_ID,TYPE,SIZE,CREATE_TIME,"\
	    "MEDIA_INFO_INDEX,DIR,FILE_UUID,MIME_TYPE,CHANGE_TIME,ACCESS_TIME) "\
        "VALUES(%u,'%s','%s',%u,%d,%lu,%u,%d,%d,'%s','%s',%u,%u);", FILE_TABLE_NAME,
         pfi->index, pfi->name_escape,"",pfi->parent_id, pfi->file_type, pfi->file_size,
         pfi->create_time,pfi->media_info_index,pfi->isFolder,pfi->file_uuid,pfi->mime_type,pfi->modify_time,pfi->access_time);
	}
...
//	DMCLOG_D("pfi->isFloder = %d",pfi->isFloder);
	...
	if(FieldValue[12] != NULL)
        pfi->access_time= strtoull(FieldValue[12], NULL, 10);
...
#endif
static error_t dm_do_delete_by_id(sqlite3 *database, uint32_t id)
{
    char *sql = get_db_write_task_sql_buf();
	error_t errcode = RET_SUCCESS;
    //delete record in file table.
...
#if 0
static error_t dm_do_delete_by_path(sqlite3 *database, const char *path)
{
    char *sql = get_db_write_task_sql_buf();
	error_t errcode = RET_SUCCESS;
    //delete record in file table.
 	sprintf(sql, "DELETE FROM %s WHERE PATH='%s'", FILE_TABLE_NAME, path);
	if((errcode = sqlite3_exec_busy_wait(database, sql, NULL, NULL))
			!= SQLITE_OK)
	{
		return errcode;
...
static error_t file_table_generic_delete(sqlite3 *database, void *target)
{
	ENTER_FUNC();
	//file_type_t type;
	...
	if((errcode = dm_do_delete_by_path(database, pdel_data))
			!= RET_SUCCESS)
	{
		return errcode;
	}
...
static error_t file_table_delete(sqlite3 *database, void *target)
{
	...
	char *path_escape = db_path_escape(pdel_data->path);
	if(path_escape == NULL)
	{
		DMCLOG_D("rename file error,src_path=%s",path_escape);
		return EDB_RECORD_NOT_EXIST;
	}
	...
	*tmp = '\0';
	...
	{
		DMCLOG_D("rename file doesn't exist,src_path=%s",path_escape);
		free(path_escape);
		return EDB_RECORD_NOT_EXIST;
	}
	*tmp = '/';
	parent_id = file_info.index;
	...
	errcode = query_file_info_by_parentid_and_name(database,parent_id,name_escape,&file_info);
	if(errcode == EDB_RECORD_NOT_EXIST)
	{
		DMCLOG_D("rename file doesn't exist,src_path=%s",path_escape);
		free(path_escape);
	...
	errcode = dm_do_delete(database,file_info.index);
...
static error_t create_full_path(sqlite3 *database, file_info_t *pfi, uint32_t *last_parent_id)
{
	...
	for(i=ROOT_LEVEL+1,parent_id=ROOT_DIR_ID;i<=level;i++)
	{
		...
		end = strstr(path,dir_name);
		if(*(end + strlen(dir_name)) == '/')
		{
		...
		pfi->path = path;
		pfi->name_escape = bb_basename(path);
		DMCLOG_D("path = %s,name_escape",path,pfi->name_escape);
...
static error_t dm_file_insert(sqlite3 *database, void *item_info)
{
	...
	errcode = get_parent_id(database, path_escape, &parent_id);
	if(errcode == EDB_RECORD_NOT_EXIST)
	{
		...
		parent_exist = FALSE;
		if((errcode = get_parent_dir(path_escape, fi_tmp.dir)) != RET_SUCCESS)
		{
		...
		fi_tmp.access_time = pfi->access_time;
	...
	file_tmp.path = path_escape;
	file_tmp.name_escape = bb_basename(path_escape);
	if(file_tmp.index == 0)
	{
        log_error("db: alloc file id error when insert %s\n",pfi->path);
		errcode = EDB_ID_OVERFLOW;
		goto EXIT;
	}
...
static error_t file_table_update_mtime_by_id(sqlite3 *database, unsigned id,
									unsigned ctime,unsigned mtime,unsigned atime)
{
	ENTER_FUNC();
	char *sql = get_db_write_task_sql_buf();
	error_t errcode = RET_SUCCESS;

	sprintf(sql,"UPDATE %s SET CREATE_TIME=%u,CHANGE_TIME=%u,ACCESS_TIME=%u WHERE ID=%d", FILE_TABLE_NAME, ctime,mtime,atime,id);
	DMCLOG_D("sql = %s",sql);
	if((errcode = sqlite3_exec_busy_wait(database, sql, NULL, NULL))
			!= RET_SUCCESS)
	{
...
#if 0
static error_t dm_dir_delete(sqlite3 *database, char *path)
{
    char *sql = get_db_write_task_sql_buf();
	error_t errcode = RET_SUCCESS;
    //delete record in file table.
 	sprintf(sql, "DELETE FROM %s WHERE PATH like '%s/%c'", FILE_TABLE_NAME,path,'%');
	if((errcode = sqlite3_exec_busy_wait(database, sql, NULL, NULL))
			!= SQLITE_OK)
	{
		DMCLOG_E("db:delete dir error path = %s",path);
		return errcode;
	}
	DMCLOG_D("db:delete dir success sql = %s",sql);
	return errcode;
}
#endif
int db_rm(sqlite3 *database,const char *path,unsigned parent_id)
{
	error_t errcode = RET_SUCCESS;
	file_list_t file_list;
	file_info_t *item,*n;
    ...
    if(file_list.result_cnt > 0)
    {
		dl_list_for_each(item, &(file_list.head), file_info_t, next)
		{
		 // init item member
			...
			if(access(full_path,F_OK) != 0)/*  Èç¹û²»´æÔÚ¾ÍÉ¾³ý*/
			{
				if(item->isFolder == 1)
				{
					//the file is folder
					
					db_rm(database,item->path,item->index);
				}
					//the file is normal file
				if((errcode = dm_do_delete_by_id(database, item->index)) != 0)
				{
...
static struct file_dnode *dm_insert(sqlite3 *database,char *fullname,unsigned parent_id,pthread_mutex_t *mutex,unsigned *commit_flag)
{
	error_t errcode = RET_SUCCESS;
	struct stat statbuf;
	...
	char *path_escape = db_path_escape(fullname + strlen(DOCUMENT_ROOT));
	if(path_escape == NULL)
	{
		DMCLOG_E("db path escape error");
		return cur;
	}
	...
	file_info.name_escape = bb_basename(path_escape);
    errcode = query_file_info_by_parentid_and_name(database,parent_id,file_info.name_escape,&file_info);
    if(errcode == EDB_RECORD_NOT_EXIST)
    {
    	file_info.path = path_escape;
    	file_info.create_time = statbuf.st_ctime;
		file_info.modify_time = statbuf.st_mtime;
		file_info.access_time = statbuf.st_atime;
        if(*commit_flag == 0)
        {
            DMCLOG_D("commit_flag = %u",*commit_flag);
            pthread_mutex_lock(mutex);
            sqlite3_exec_busy_wait(database, "begin", NULL, NULL);
        }
        if((errcode = dm_scan_file_insert(database,&file_info)) != RET_SUCCESS)
        {
        ...
        (*commit_flag)++;
        if(*commit_flag == COMMIT_COUNT)//æ¯æ’å…¥COMMIT_COUNTæ¡æ•°æ®commitä¸€ä¸ªä»»åŠ¡
        {
            DMCLOG_D("commit_flag = %u",*commit_flag);
            *commit_flag = 0;
            sqlite3_exec_busy_wait(database, "commit", NULL, NULL);
    ...
    }else if(errcode == RET_SUCCESS){
		if(file_info.isFolder == 1)
		{
			if(file_info.modify_time != statbuf.st_mtime)// the  dir has changed
			{
				DMCLOG_D("db path : %s changed",file_info.path);
				file_table_update_mtime_by_id(database, file_info.index,statbuf.st_ctime,statbuf.st_mtime,statbuf.st_atime);
				db_rm(database,file_info.path,file_info.index);
			}
		}
		if(file_info.path != NULL)
		{
...
static struct file_dnode **scan_one_dir(sqlite3 *database,const char *path, unsigned parent_id,unsigned *nfiles_p,pthread_mutex_t *mutex,unsigned *commit_flag)
{
	...
	*nfiles_p = 0;
	...
	while ((entry = readdir(dir)) != NULL) {
		...
		cur = dm_insert(database,fullname,parent_id,mutex,commit_flag);
		if (!cur) {
			free(fullname);
			goto EXIT;
		}
...
static void dm_scan_and_display_dirs_recur(sqlite3 *database,struct file_dnode **dn,pthread_mutex_t *mutex,unsigned *commit_flag)
{
	unsigned nfiles;
	struct file_dnode **subdnp;
	for (; *dn; dn++) {
		//DMCLOG_D("start:%s:", (*dn)->fullname);
		subdnp = scan_one_dir(database,(*dn)->fullname, (*dn)->index,&nfiles,mutex,commit_flag);
		if (nfiles > 0) {
			...
			if (dndirs > 0) {
				dm_scan_and_display_dirs_recur(database,dnd,mutex,commit_flag);
...
static void scan_and_display_dirs_recur(sqlite3 *database,struct file_dnode **dn,pthread_mutex_t *mutex,unsigned *commit_flag)
{
	unsigned nfiles;
	struct file_dnode **subdnp;
	for (; *dn; dn++) {
		...
		if (nfiles > 0) {
			...
			if (dndirs > 0) {
				scan_and_display_dirs_recur(database,dnd,mutex,commit_flag);
				/* free the array of dnode pointers to the dirs */
				free(dnd);
			}
			/* free the dnodes and the fullname mem */
...
static error_t file_table_scan(sqlite3 *database, void *path_info)
{
	...
	memset(&file_info,0,sizeof(file_info_t));
    errcode = query_file_info(database,pfi->disk_path + strlen(DOCUMENT_ROOT),&file_info);
    if(errcode == EDB_RECORD_NOT_EXIST)
    {
    	file_info.path = pfi->disk_path + strlen(DOCUMENT_ROOT);
		file_info.name = bb_basename(pfi->disk_path);
		file_info.name_escape = file_info.name ;
		file_info.file_size = statbuf.st_size;
		file_info.create_time = statbuf.st_ctime ;
		file_info.modify_time = statbuf.st_mtime;
		file_info.access_time = statbuf.st_atime;
        ...
        pthread_mutex_lock(pfi->mutex);
        sqlite3_exec_busy_wait(database, "begin", NULL, NULL);
        if((errcode = sqlite3_exec_busy_wait(database, sql, NULL, NULL))!= RET_SUCCESS)
        {
            log_warning("db:insert %s error\n", file_info.path);
            sqlite3_exec_busy_wait(database, "commit", NULL, NULL);
            pthread_mutex_unlock(pfi->mutex);
            return errcode;
        }
        sqlite3_exec_busy_wait(database, "commit", NULL, NULL);
        pthread_mutex_unlock(pfi->mutex);
		dm_scan_and_display_dirs_recur(database,dnp,pfi->mutex,&commit_flag);
	    if(commit_flag != 0&&commit_flag != COMMIT_COUNT)
	    {
	        sqlite3_exec_busy_wait(database, "commit", NULL, NULL);
	        pthread_mutex_unlock(pfi->mutex);
	    }
    	...
    	dn.index = file_info.index;
		//judge the dir whether or not be changed
		if(file_info.modify_time != statbuf.st_mtime)// the root dir has changed
		{
			DMCLOG_D(" the root dir has changed");
			pthread_mutex_lock(pfi->mutex);
        	sqlite3_exec_busy_wait(database, "begin", NULL, NULL);
			file_table_update_mtime_by_id(database, file_info.index,statbuf.st_ctime,statbuf.st_mtime,statbuf.st_atime);
			sqlite3_exec_busy_wait(database, "commit", NULL, NULL);
		...
		pthread_mutex_lock(pfi->mutex);
        sqlite3_exec_busy_wait(database, "begin", NULL, NULL);
		errcode = db_rm(database,file_info.path,file_info.index);
		sqlite3_exec_busy_wait(database, "commit", NULL, NULL);
        pthread_mutex_unlock(pfi->mutex);
		DMCLOG_D("file_table_scan finish");
	}
	EXIT_FUNC();
	return 0;
}
									...
									const char *des_path)
{
	...
	if(isfolder == 1)
		sprintf(sql,"UPDATE %s SET NAME='%s',PATH='%s' WHERE ID=%d", FILE_TABLE_NAME, bb_basename(path_escape),path_escape,id);
	else
		sprintf(sql,"UPDATE %s SET NAME='%s' WHERE ID=%d", FILE_TABLE_NAME, bb_basename(path_escape),id);
	if((errcode = sqlite3_exec_busy_wait(database, sql, NULL, NULL))
			!= RET_SUCCESS)
	{
		if(path_escape != NULL)
...
static error_t file_table_rename(sqlite3 *database, char *src_path,char *des_path)
{
	...
	char *path_escape = db_path_escape(src_path);
	if(path_escape == NULL)
	{
		DMCLOG_D("rename file error,src_path=%s",src_path);
		return EDB_RECORD_NOT_EXIST;
	}
	char *tmp = strrchr(path_escape,'/');
	*tmp = '\0';
	...
	*tmp = '/';
	...
	if(file_info.name != NULL)
		free(file_info.name);
	file_info.name_escape = bb_basename(path_escape);
    errcode = query_file_info_by_parentid_and_name(database,file_info.index,file_info.name_escape,&file_info);
	if(errcode == EDB_RECORD_NOT_EXIST)
	{
	...
	free(path_escape);
	return file_table_rename_by_id(database, file_info.index,file_info.isFolder,des_path);
}
