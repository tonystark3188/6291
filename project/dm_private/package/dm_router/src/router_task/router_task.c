...
int get_info_from_hd_list(_In_ const char *session,_Out_ char *ip,_Out_ int *port)
{
    unsigned i = 0;
    int res = -1;
    if(router_dn == NULL || session == NULL || ip == NULL)
        return res;
    for(i = 0;/*i < counr detected via !dn below*/;i++)
    {
        DMCLOG_D("i = %d,session = %s,ip = %s,port = %d",i,router_dn->session,router_dn->ip,router_dn->port);
        if(!strcmp(session,router_dn->session))
        {
            strcpy(ip,router_dn->ip);
            *port = router_dn->port;
            res = 0;
            goto exit;
        }
        router_dn = router_dn->dn_next;
        if(!router_dn)
            break;
    }
exit:
