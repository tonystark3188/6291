...
int set_forget_wifi_info(char *mac, struct conn *c)
{
	JObj* response_json=JSON_NEW_EMPTY_OBJECT();
	JObj* header_json=JSON_NEW_EMPTY_OBJECT();
	int ret = dm_set_forget_wifi_info(mac);
	if(ret < 0)
	{
		c->error = dm_router_errors(ret);
	}
...
int _dm_set_forget_wifi_info(struct conn *c)
{
	...
	if(mac_json != NULL)
	{
		int ret = set_forget_wifi_info(JSON_GET_OBJECT_VALUE(mac_json,string),c);
		if(ret < 0)
		{	
			goto EXIT;
		}
...
{	
	...
	c->src_path = (char *)malloc(strlen(FW_FILE) + 1);
	if(c->src_path == NULL)
	{
		DMCLOG_D("malloc error");
		res = -1;
	...
	c->tmp_path = (char *)malloc(strlen(c->src_path)+strlen(TMP_PATH_NAME)+1);
	if(c->tmp_path == NULL)
	{
		DMCLOG_D("malloc error");
		res = -1;
	...
	sprintf(c->tmp_path,"%s%s", c->src_path, TMP_PATH_NAME);
