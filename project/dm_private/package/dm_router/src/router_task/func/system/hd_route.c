...
{
	DMCLOG_D("access dm_get_ota_info");
	int ret =0; 
	FILE *read_fp = NULL;
	char tmp_mac[32]="\0";
...
int dm_upgrade_fw()
{
	...
	char cmd[128];
			...
			system("dd if=/tmp/fwupgrade_1 of=/dev/mmcblk0 bs=1M count=5 seek=3");
			system("sync");
			system("dd if=/tmp/fwupgrade_1 of=/dev/mmcblk0 bs=1M skip=5 seek=8");
			system("sync");
			system("reboot -f");
		#endif
	#endif
