...
int get_wifi_settings(hd_wifi_info *m_wifi_info)
{
	...
	strcpy(ifname,"wlan0");
	cgi_get_channel(ifname, m_wifi_info->channel); 
...
{	
	...
	{
		if( !memcmp( (wifilist+i)->ssid, m_remote_info->ssid ,strlen(m_remote_info->ssid)) )
		{
	...
	if(flag)
	{
		sprintf(cmd_buf,"uci set wifilist.@wifi%dg[%d].ssid=%s",iwifimode,i,m_remote_info->ssid);
		//uci_set_option_value(str_sp);
		system(cmd_buf);
		memset(cmd_buf,0,TEMP_BUFFER_SIZE);
		sprintf(cmd_buf,"uci set wifilist.@wifi%dg[%d].bssid=%s",iwifimode,i,macaddr);
		//uci_set_option_value(str_sp);
		system(cmd_buf);
		memset(cmd_buf,0,TEMP_BUFFER_SIZE);
		...
		{
			sprintf(cmd_buf,"uci set wifilist.@wifi%dg[%d].encryption=%s",iwifimode,i,encrypt_config);
		}
		else
		{
			sprintf(cmd_buf,"uci set wifilist.@wifi%dg[%d].encryption=%s+%s",iwifimode,i,encrypt_config,tkip_aes_config);
		}
		//uci_set_option_value(str_sp);
		system(cmd_buf);
		memset(cmd_buf,0,TEMP_BUFFER_SIZE);
		...
		{
			sprintf(cmd_buf,"uci set wifilist.@wifi%dg[%d].key=%s",iwifimode,i,m_remote_info->password);
		}
		//uci_set_option_value(str_sp);
		system(cmd_buf);
		memset(cmd_buf,0,TEMP_BUFFER_SIZE);
	...
	{
		sprintf(cmd_buf,"uci add wifilist wifi%dg",iwifimode);
		system(cmd_buf);
		memset(cmd_buf,0,TEMP_BUFFER_SIZE);
		sprintf(cmd_buf,"uci set wifilist.@wifi%dg[-1].ssid='%s'",iwifimode,m_remote_info->ssid);
		system(cmd_buf);
		memset(cmd_buf,0,TEMP_BUFFER_SIZE);
		sprintf(cmd_buf,"uci set wifilist.@wifi%dg[-1].bssid='%s'",iwifimode,macaddr);
		system(cmd_buf);
		memset(cmd_buf,0,TEMP_BUFFER_SIZE);
		if(!strcmp(encrypt_config,"none") || !strcmp(encrypt_config,"wep"))
		{
			sprintf(cmd_buf,"uci set wifilist.@wifi%dg[-1].encryption=%s",iwifimode,encrypt_config);
		}
		else
		{
			sprintf(cmd_buf,"uci set wifilist.@wifi%dg[-1].encryption=%s+%s",iwifimode,encrypt_config,tkip_aes_config);
		}
		system(cmd_buf);
		memset(cmd_buf,0,TEMP_BUFFER_SIZE);
		if(strcmp(encrypt_config,"none")!=0)
		{
			sprintf(cmd_buf,"uci set wifilist.@wifi%dg[-1].key='%s'",iwifimode,m_remote_info->password);
		}
		system(cmd_buf);
		memset(cmd_buf,0,TEMP_BUFFER_SIZE);
		sprintf(cmd_buf,"uci set wifilist.number%dg.num=%d",iwifimode,wifinumber+1);
		system(cmd_buf);
		memset(cmd_buf,0,TEMP_BUFFER_SIZE);
   ...
   //system("uci commit");
	...
	#elif defined(OPENWRT_X1000)
	//system("`sleep 3;set_client.sh >/dev/null 2>&1` &");
	system("`sleep 3;control_dns.sh >/dev/null 2>&1` &");
	#endif
#elif defined(SUPPORT_LINUX_PLATFORM)
#endif
    return 0;
...
int _set_client_status(int status)
{
	...
	system("`sleep 3;control_dns.sh auto` &");
	#endif
#elif defined(SUPPORT_LINUX_PLATFORM)
#endif
	return ROUTER_OK;
...
int _set_forget_wifi_info(char *pbssid)
{
	...
	for(i = 0;i < tmp_sum; i++){
		...
		if(strcasecmp(wifilist_mac,pbssid) == 0)
		{
			memset(uci_option_str,0,sizeof(uci_option_str));
			memset(disabled,0,sizeof(disabled));
			sprintf(uci_option_str,"remoteAPlist.@ap[%d].disabled",i);
			...
			if(strcmp(disabled,"1") == 0){
				DMCLOG_M("mac of %s has already forgot", pbssid);
			}
			else{
				memset(uci_option_str, 0, sizeof(uci_option_str));
				...
				if(strcasecmp(wireless_mac,pbssid) == 0)
				{
					system("iwpriv apcli0 set ApCliEnable=0");
					system("ifconfig apcli0 down");
				}
	...
	for(i = 0; i < ap_sum; i++){
		if(wifimode==M2G){
			memset(wifilist_mac, 0, sizeof(wifilist_mac));
			memset(uci_option_str, 0, sizeof(uci_option_str));
			sprintf(uci_option_str,"wifilist.@wifi2g[%d].bssid",i);
			ret = uci_get_option_value(uci_option_str, wifilist_mac);
			if(ret < 0){
				return ROUTER_ERRORS_UCI;
			}
			...
			if(strcasecmp(wifilist_mac,pbssid) == 0){
				...
				memset(uci_option_str, 0, sizeof(uci_option_str));
				memset(wireless_mac, 0, sizeof(wireless_mac));
				sprintf(uci_option_str,"wireless.@wifi-iface[1].bssid");
				ret = uci_get_option_value(uci_option_str,wireless_mac);	
				if(ret < 0){
					return ROUTER_ERRORS_UCI;
				}
				if(strcasecmp(wireless_mac,pbssid) == 0)
			...
			memset(wifilist_mac, 0, sizeof(wifilist_mac));
			memset(uci_option_str, 0, sizeof(uci_option_str));
			sprintf(uci_option_str,"wifilist.@wifi5g[%d].bssid",i);
			ret = uci_get_option_value(uci_option_str, wifilist_mac);
			if(ret < 0){
				return ROUTER_ERRORS_UCI;
			}
