...
{
	if(!(stream->conn->cmd == FN_FILE_UPLOAD|| stream->conn->cmd == FN_FILE_GET_BACKUP_FILE))
	{
...
dm_send_file_server(struct conn *c)
{
	...
	ret = file_process(c);
	if(ret < 0){
		DMCLOG_D("STOP STOP ret = %d", ret);
		io_clear(&c->loc.io);
		c->loc.headers_len = c->loc.io.head = my_snprintf(c->loc.io.buf,
	    c->loc.io.size, "{\"header\": { \"cmd\": %d, \"seq\": 0, \"error\": %d } }",
	    c->cmd,c->error);
		stop_stream(&c->rem);
	}
...
disconnect(struct conn *c)
{
	...
	if (dont_close) {
		...
		if((c->cmd == FN_FILE_UPLOAD|| c->cmd == FN_FILE_GET_BACKUP_FILE || c->cmd == FN_ROUTER_SET_UPLOAD_FIRMWARE)&&c->rem.io.total == c->rem.content_len)
		{
				...
				if(c->dn == NULL)
				{
					...
					if(c->cmd != FN_ROUTER_SET_UPLOAD_FIRMWARE)
					{
					    ...
					    if (stat(path, &statbuf) == -1) {  
					        DMCLOG_E("stat error for %s", path);  
					    } else {  
					    	if(c->modifyTime != 0)
				    		{
								timebuf.modtime = c->modifyTime; /*最后修改时间*/  
						        timebuf.actime = c->modifyTime;/*最后访问时间改为最后修改时间*/  
						        if (utime(path, &timebuf) == -1)   /*修改时间*/  
									DMCLOG_E("utime error for %s", path);  
							}
					    }  
						#endif
			...
			char send_buf[1024] = {0};
			dm_file_upload_inotify(c->cmd, send_buf);
...
shttpd_poll(struct shttpd_ctx *ctx, int milliseconds)
{
	...
	/* get time for record */
	now_time = time(NULL);
	//DMCLOG_M("now_time = %lu, c->record_time = %lu", now_time, c->record_time);
		    ...
		    io_space_len(&c->rem.io))
		{
			...
			if (!(c->rem.flags & FLAG_HEADERS_PARSED))
			{
				...
				if(!(c->rem.flags & FLAG_CLOSED))
				{
					...
					{
						if(c->cmd != FN_FILE_GET_LSIT_BY_TYPE&&c->cmd != FN_FILE_GET_DIR_BY_TYPE&&c->cmd != FN_FILE_GET_LIST_BY_PATH&&(c->cmd != FN_FILE_GET_LIST || c->pageNum >= 0))
		    ...
		    c->loc.io_class != NULL && c->loc.io_class->write != NULL)
		{
			...
			if(now_time - c->record_time > 0)
			{
				...
				if(ret >= 0)
				{
					...
					DMCLOG_D("access record cur_position = %lu,c->length = %lu",cur_position,c->length);
