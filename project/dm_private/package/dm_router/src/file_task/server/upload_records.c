...
*/
int del_record_from_list_for_index(struct record_dnode **head,unsigned long end)  
{	
	ENTER_FUNC();
 ...
 void display_record_dnode(struct record_dnode *dn)
 {
	 ...
	 for(i = 0;/*count - detected via !dn below*/;i++)
	 {
		 ...
		 DMCLOG_D("i = %d,dn->index = %d,dn->start = %lu,dn->end = %lu",i,dn->index,dn->start,dn->end);
		 dn = dn->dn_next;
	 }
 }
 
int write_list_to_file(const char *path,struct record_dnode *dn)//写出数据
{
	unsigned i = 0;
	int enRet = 0;
	FILE *record_fd;
	...
	for(i = 0;/*count - detected via !dn below*/;i++)
	{
		...
		DMCLOG_D("i = %d,dn->index = %d",i,dn->index);
		enRet = fwrite(dn,sizeof(struct record_dnode),1,record_fd);
		if(enRet <= 0)
		{
			DMCLOG_D("enRet = %d,errno = %d",enRet,errno);
		}
		dn = dn->dn_next;
	}
	fclose(record_fd);
 ...
 int read_list_from_file(const char *path,struct record_dnode **dn)//读入数据
 {	 
	  FILE *record_fd;
	  int enRet;
	  ...
	  {
		 cur = xzalloc(sizeof(*cur));
		 enRet = fread(cur,sizeof(struct record_dnode),1,record_fd);
		 if(enRet <= 0)
	     {
			DMCLOG_D("fread error,enRet = %d,errno = %d",enRet,errno);
			break;
		 }
		 DMCLOG_D("i:%d,index = %d,end = %ld",i,cur->index,cur->start,cur->end);
 ...
 int update_record_for_index(struct record_dnode **head,unsigned long end,unsigned long cur)
 {
	 ...
	 else
	 {
		 if (node1->end == end)
		 {
			 DMCLOG_D("node1->end = %lu",node1->end);
			 node1->start = cur;
