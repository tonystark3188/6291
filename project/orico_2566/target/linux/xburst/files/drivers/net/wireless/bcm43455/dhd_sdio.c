...
dhdsdio_download_firmware(struct dhd_bus *bus, osl_t *osh, void *sdh)
{
	...
	chip = bus->dhd->conf->chip;
	chiprev = bus->dhd->conf->chiprev;
	...
	if(chip == BCM43430_CHIP_ID) {
		if(chiprev == BCM43430A0_CHIP_REV) {
			strcpy(bus->nv_path,  "/system/etc/firmware/nvram_bcm43438a0.txt");
		}
		else if(chiprev == BCM43430A1_CHIP_REV) {
			strcpy(bus->nv_path,  "/system/etc/firmware/nvram_bcm43438a1.txt");
		}
	}
