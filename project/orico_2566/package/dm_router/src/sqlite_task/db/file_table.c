...
static error_t file_table_update(sqlite3 *database, void *update_info)
{
	...
	char *path_escape = db_path_escape(file_update->file_info.path);
	if(path_escape == NULL)
	{
		DMCLOG_E("rename file error,src_path=%s",path_escape);
		return EDB_RECORD_NOT_EXIST;
	}
	char *tmp = strrchr(path_escape,'/');
	if(tmp == NULL)
	...
	errcode = query_file_id_by_parentid_and_name(database,parent_id,name_escape,&file_update->file_info.index);
	if(errcode == EDB_RECORD_NOT_EXIST)
	{
		DMCLOG_D("query file doesn't exist,src_path=%s",path_escape);
		safe_free(path_escape);
		return EDB_RECORD_NOT_EXIST;
	}
