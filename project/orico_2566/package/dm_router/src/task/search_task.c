		...
		//DMCLOG_D("ent->d_name: %s", entry->d_name);
			...
			if(entry->d_type == 8)  // d_type：8-文件，4-目录
			{
				if(NULL != strstr(name_tolower, search_str_tolower)){
					...
					//file_info->path = strdup(fullname);
			...
			else
			{
				if(NULL != strstr(name_tolower, search_str_tolower)){
					...
					file_info->path = (char *)calloc(1, strlen(fullname)-strlen(DOCUMENT_ROOT)+1);
					if(file_info->path != NULL){
						memcpy(file_info->path, fullname+strlen(DOCUMENT_ROOT), strlen(fullname)-strlen(DOCUMENT_ROOT));
					}
			...
			//if((pInfo->list_nfiles >= SEND_LIST_INFO_MIN) || ((pInfo->list_nfiles >= 1) && ((cur_time - pInfo->record_time) >= SEND_LIST_INFO_TIME))){
			if((pInfo->list_nfiles >= SEND_LIST_INFO_MIN) || ((cur_time - pInfo->record_time) >= SEND_LIST_INFO_TIME)){				
				...
				if(ret < 0){
					...
					pInfo->send_error_times++;
				}
				else{
					//DMCLOG_D("search_handle_udp_inotify success");
					pInfo->send_error_times = 0;
...
int search_file_traversal(file_search_info_t *pInfo, struct file_dnode **dn, file_list_t *plist)
{
	...
	for (; (*dn)&&(!(*pInfo->flags & FLAG_CLOSED)); dn++) {
		...
		//DMCLOG_D("nfiles_dir: %d", nfiles_dir);
		usleep(10);
		if(nfiles_dir > 0){
...
{
    ENTER_FUNC();
    file_search_info_t *pInfo = (copy_info_t *)self;
    //char *search_string = pInfo->search_string;
	...
	#ifdef SEARCH_TASK_TCP
	while((pInfo->list_nfiles > 0) && (pInfo->send_error_times < SEND_ERROR_TIMES_MAX) && (!(*pInfo->flags & FLAG_CLOSED))){
		if(pInfo->list_nfiles >= SEND_LIST_INFO_MAX){
			pInfo->send_nfiles = SEND_LIST_INFO_MAX;
		}
		else{
