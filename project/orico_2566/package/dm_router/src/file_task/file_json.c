...
int create_search_task(struct conn *c)
{
	...
	pInfo->c = c;
    pInfo->flags = &c->rem.flags;
    //pInfo->sock = c->loc.chan.sock;
    pInfo->cmd = c->cmd;
    strcpy(pInfo->session,c->session);
	strcpy(pInfo->ip,c->client_ip);
    pInfo->search_seq = c->search_seq;
	//strcpy(pInfo->disk_uuid,c->disk_uuid);
    pInfo->search_path = (char *)calloc(1,strlen(c->search_path) + 1);
    strcpy(pInfo->search_path,c->search_path);
    pInfo->search_string = (char *)calloc(1,strlen(c->search_string) + 1);
    strcpy(pInfo->search_string,c->search_string);
...
int DM_FileSearch(struct conn *c)
{
	...
	if(res < 0)
	{
		DMCLOG_D("search (%s) for (%s) failed", c->search_string, c->search_path);
...
int parser_FileSearch(struct conn *c)
{
    ...
    c->search_seq = JSON_GET_OBJECT_VALUE(seq_json,int);
	...
	c->search_string = (char *)calloc(1, strlen(searchString) + 2);
	if(c->search_string == NULL)
    {
        DMCLOG_D("malloc error");
        c->error = SERVER_OUT_MEMORY;
        goto EXIT;
    ...
    DMCLOG_D("srcPath = %s",searchPath);
    if (strlen(searchPath) + strlen(DOCUMENT_ROOT) >= URI_MAX) {
        c->error = PATH_TOO_LONGTH;
        goto EXIT;
    }
    c->search_path = (char *)calloc(1, strlen(searchPath) + strlen(DOCUMENT_ROOT) + 2);
    if(c->search_path == NULL)
    {
        DMCLOG_D("malloc error");
        c->error = SERVER_OUT_MEMORY;
        goto EXIT;
    ...
    if(*searchPath == '/')
    {
        sprintf(c->search_path, "%s%s",DOCUMENT_ROOT,searchPath);
    }else{
        sprintf(c->search_path, "%s/%s",DOCUMENT_ROOT,searchPath);
    }
    DMCLOG_D("c->search_path = %s",c->search_path);
EXIT:
    if(c->r_json != NULL)
        JSON_PUT_OBJECT(c->r_json);
    return 0;	
