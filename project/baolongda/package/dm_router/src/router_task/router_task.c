...
int _get_info_from_dev_list(_In_ const char *session,_Out_ char *ip,_Out_ int *port)
{
	...
	{
		if(!strcmp(dev_dnode->session,session))
		{
			if(*dev_dnode->ip&&dev_dnode->port != 0)
        	{
				strcpy(ip,dev_dnode->ip);
            	*port = dev_dnode->port;
			}else{
				if(get_ip_from_usr_table(session,ip) < 0)
				{
					DMCLOG_D("get ip error");
					pthread_mutex_unlock(&p_dev_list->mutex);
					return -1;
				}
				strcpy(dev_dnode->ip,ip);
				dev_dnode->port = router_para_listen_port;
				dev_dnode->request_type = router_request_type;
				*port = router_para_listen_port;
			}
			pthread_mutex_unlock(&p_dev_list->mutex);
			return 0;
		}
...
int _update_dev_to_list(char *session_id,char *ip,uint16_t port,uint8_t request_type)
{
	dev_list_t *p_dev_list = &dev_list;
	if(session_id == NULL || ip == NULL)
	{
		DMCLOG_E("para error");
		return -1;
	}
	...
	dl_list_for_each(dev_dnode, &(p_dev_list->head), dev_dnode_t, next)
	{
		...
		{
			DMCLOG_D("the ip is exist");
			strcpy(dev_dnode->session,session_id);
			dev_dnode->port = port;
...
int _add_dev_to_list(char *session_id,uint16_t port,uint8_t request_type)
{
	dev_list_t *p_dev_list = &dev_list;
	if(session_id == NULL)
	{
	...
	char ip[32];
	memset(ip,0,32);
	int res = get_ip_from_usr_table(session_id,ip);
	if(res < 0)
	{
		DMCLOG_D("get ip error");
		return -1;
	...
	if(is_dev_exist(ip))
	{
		...
		_update_dev_to_list(session_id,ip,port,request_type);
		return 0;
	}
	DMCLOG_D("ip = %s",ip);
	pthread_mutex_lock(&p_dev_list->mutex);
