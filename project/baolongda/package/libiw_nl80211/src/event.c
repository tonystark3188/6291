...
static void print_frame(struct print_event_args *args, struct nlattr *attr)
{
	...
	case 0x30: /* reassoc resp */
		...
		p_debug(" status: %d: %s", tmp, get_status_str(tmp));
		break;
	case 0x00: /* assoc req */
	...
	case 0xc0: /* deauth */
		...
		p_debug(" reason %d: %s", tmp, get_reason_str(tmp));
		break;
	}
...
static void parse_cqm_event(struct nlattr **attrs)
{
	...
	if (cqm[NL80211_ATTR_CQM_RSSI_THRESHOLD_EVENT]) {
		...
		case NL80211_CQM_RSSI_THRESHOLD_EVENT_HIGH:
			p_debug("RSSI went above threshold\n");
			found_one = true;
			break;
		case NL80211_CQM_RSSI_THRESHOLD_EVENT_LOW:
			p_debug("RSSI went below threshold\n");
			found_one = true;
			break;
		case NL80211_CQM_RSSI_BEACON_LOSS_EVENT:
			p_debug("Beacon loss detected\n");
			found_one = true;
			break;
		}

		if (!found_one)
			p_debug("Unknown event type: %i\n", rssi_event);
		...
		p_debug("peer %s didn't ACK %d packets\n", buf, frames);
	} else
		p_debug("unknown event\n");
}
...
static void parse_wowlan_wake_event(struct nlattr **attrs)
{
	...
	p_debug("WoWLAN wakeup\n");
	if (!attrs[NL80211_ATTR_WOWLAN_TRIGGERS]) {
		p_debug("\twakeup not due to WoWLAN\n");
		return;
	}
	...
	if (tb[NL80211_WOWLAN_TRIG_DISCONNECT])
		p_debug("\t* was disconnected\n");
	if (tb[NL80211_WOWLAN_TRIG_MAGIC_PKT])
		p_debug("\t* magic packet received\n");
	if (tb[NL80211_WOWLAN_TRIG_PKT_PATTERN])
		p_debug("\t* pattern index: %u\n",
		       nla_get_u32(tb[NL80211_WOWLAN_TRIG_PKT_PATTERN]));
	if (tb[NL80211_WOWLAN_TRIG_GTK_REKEY_FAILURE])
		p_debug("\t* GTK rekey failure\n");
	if (tb[NL80211_WOWLAN_TRIG_EAP_IDENT_REQUEST])
		p_debug("\t* EAP identity request\n");
	if (tb[NL80211_WOWLAN_TRIG_4WAY_HANDSHAKE])
		p_debug("\t* 4-way handshake\n");
	if (tb[NL80211_WOWLAN_TRIG_RFKILL_RELEASE])
		p_debug("\t* RF-kill released\n");
	if (tb[NL80211_WOWLAN_TRIG_WAKEUP_PKT_80211]) {
		...
		int i;
		p_debug("\t* packet (might be truncated): ");
		for (i = 0; i < l; i++) {
			if (i > 0)
				p_debug(":");
			p_debug("%.2x", d[i]);
	...
	if (tb[NL80211_WOWLAN_TRIG_WAKEUP_TCP_MATCH])
		p_debug("\t* TCP connection wakeup received\n");
	if (tb[NL80211_WOWLAN_TRIG_WAKEUP_TCP_CONNLOST])
		p_debug("\t* TCP connection lost\n");
	if (tb[NL80211_WOWLAN_TRIG_WAKEUP_TCP_NOMORETOKENS])
		p_debug("\t* TCP connection ran out of tokens\n");
}
...
static int print_event(struct nl_msg *msg, void *arg)
{
	...
	if (args->time || args->reltime) {
		...
		p_debug("%llu.%06llu: ", usecs/1000000, usecs % 1000000);
	}
	...
	if (tb[NL80211_ATTR_IFINDEX] && tb[NL80211_ATTR_WIPHY]) {
		...
		p_debug("%s (phy #%d): ", ifname, nla_get_u32(tb[NL80211_ATTR_WIPHY]));
	} else if (tb[NL80211_ATTR_WDEV] && tb[NL80211_ATTR_WIPHY]) {
		p_debug("wdev 0x%llx (phy #%d): ",
		...
		p_debug("wdev 0x%llx: ", (unsigned long long)nla_get_u64(tb[NL80211_ATTR_WDEV]));
	} else if (tb[NL80211_ATTR_WIPHY]) {
		p_debug("phy #%d: ", nla_get_u32(tb[NL80211_ATTR_WIPHY]));
	}

	switch (gnlh->cmd) {
	case NL80211_CMD_NEW_WIPHY:
		p_debug("renamed to %s\n", nla_get_string(tb[NL80211_ATTR_WIPHY_NAME]));
		break;
	case NL80211_CMD_TRIGGER_SCAN:
		p_debug("scan started\n");
		break;
	case NL80211_CMD_NEW_SCAN_RESULTS:
		p_debug("scan finished:");
	case NL80211_CMD_SCAN_ABORTED:
		if (gnlh->cmd == NL80211_CMD_SCAN_ABORTED)
			p_debug("scan aborted:");
		if (tb[NL80211_ATTR_SCAN_FREQUENCIES]) {
			nla_for_each_nested(nst, tb[NL80211_ATTR_SCAN_FREQUENCIES], rem_nst)
				p_debug(" %d", nla_get_u32(nst));
			p_debug(",");
		}
	...
	case NL80211_CMD_REG_CHANGE:
		p_debug("regulatory domain change: ");

		reg_type = nla_get_u8(tb[NL80211_ATTR_REG_TYPE]);

		switch (reg_type) {
		case NL80211_REGDOM_TYPE_COUNTRY:
			p_debug("set to %s by %s request",
			       nla_get_string(tb[NL80211_ATTR_REG_ALPHA2]),
			       reg_initiator_to_string(nla_get_u8(tb[NL80211_ATTR_REG_INITIATOR])));
			if (tb[NL80211_ATTR_WIPHY])
				p_debug(" on phy%d", nla_get_u32(tb[NL80211_ATTR_WIPHY]));
			break;
		case NL80211_REGDOM_TYPE_WORLD:
			p_debug("set to world roaming by %s request",
			       reg_initiator_to_string(nla_get_u8(tb[NL80211_ATTR_REG_INITIATOR])));
			break;
		case NL80211_REGDOM_TYPE_CUSTOM_WORLD:
			p_debug("custom world roaming rules in place on phy%d by %s request",
			       nla_get_u32(tb[NL80211_ATTR_WIPHY]),
			       reg_initiator_to_string(nla_get_u32(tb[NL80211_ATTR_REG_INITIATOR])));
			break;
		case NL80211_REGDOM_TYPE_INTERSECTION:
			p_debug("intersection used due to a request made by %s",
			       reg_initiator_to_string(nla_get_u32(tb[NL80211_ATTR_REG_INITIATOR])));
			if (tb[NL80211_ATTR_WIPHY])
				p_debug(" on phy%d", nla_get_u32(tb[NL80211_ATTR_WIPHY]));
			break;
		default:
			p_debug("unknown source (upgrade this utility)");
			break;
		}
	...
	case NL80211_CMD_REG_BEACON_HINT:
		...
		if (chan_before_beacon.no_ir && !chan_after_beacon.no_ir) {
			if (chan_before_beacon.no_ibss && !chan_after_beacon.no_ibss)
				p_debug("\to Initiating radiation enabled\n");
			else
				p_debug("\to active scan enabled\n");
		} else if (chan_before_beacon.no_ibss && !chan_after_beacon.no_ibss) {
			p_debug("\to ibss enabled\n");
		}

		break;
	case NL80211_CMD_NEW_STATION:
		mac_addr_n2a(macbuf, nla_data(tb[NL80211_ATTR_MAC]));
		p_debug("new station %s\n", macbuf);
		break;
	case NL80211_CMD_DEL_STATION:
		mac_addr_n2a(macbuf, nla_data(tb[NL80211_ATTR_MAC]));
		p_debug("del station %s\n", macbuf);
		break;
	case NL80211_CMD_JOIN_IBSS:
		mac_addr_n2a(macbuf, nla_data(tb[NL80211_ATTR_MAC]));
		p_debug("IBSS %s joined\n", macbuf);
		break;
	...
	case NL80211_CMD_UNPROT_DEAUTHENTICATE:
		p_debug("unprotected deauth");
		print_frame(args, tb[NL80211_ATTR_FRAME]);
		p_debug("\n");
		break;
	case NL80211_CMD_UNPROT_DISASSOCIATE:
		p_debug("unprotected disassoc");
		print_frame(args, tb[NL80211_ATTR_FRAME]);
		p_debug("\n");
		break;
	case NL80211_CMD_CONNECT:
		...
		if (status)
			p_debug(", status: %d: %s", status, get_status_str(status));
		p_debug("\n");
		break;
	...
	case NL80211_CMD_DISCONNECT:
		...
		if (tb[NL80211_ATTR_DISCONNECTED_BY_AP])
			...
			p_debug(" (local request)");
		if (tb[NL80211_ATTR_REASON_CODE])
			p_debug(" reason: %d: %s", nla_get_u16(tb[NL80211_ATTR_REASON_CODE]),
				get_reason_str(nla_get_u16(tb[NL80211_ATTR_REASON_CODE])));
		p_debug("\n");
		break;
	case NL80211_CMD_REMAIN_ON_CHANNEL:
		p_debug("remain on freq %d (%dms, cookie %llx)\n",
			nla_get_u32(tb[NL80211_ATTR_WIPHY_FREQ]),
			nla_get_u32(tb[NL80211_ATTR_DURATION]),
			(unsigned long long)nla_get_u64(tb[NL80211_ATTR_COOKIE]));
		break;
	case NL80211_CMD_CANCEL_REMAIN_ON_CHANNEL:
		p_debug("done with remain on freq %d (cookie %llx)\n",
			nla_get_u32(tb[NL80211_ATTR_WIPHY_FREQ]),
			(unsigned long long)nla_get_u64(tb[NL80211_ATTR_COOKIE]));
		break;
	...
	case NL80211_CMD_FRAME_TX_STATUS:
		p_debug("mgmt TX status (cookie %llx): %s\n",
			(unsigned long long)nla_get_u64(tb[NL80211_ATTR_COOKIE]),
			tb[NL80211_ATTR_ACK] ? "acked" : "no ack");
		break;
	case NL80211_CMD_PMKSA_CANDIDATE:
		p_debug("PMKSA candidate found\n");
		break;
	...
	case NL80211_CMD_PROBE_CLIENT:
		if (tb[NL80211_ATTR_MAC])
			...
			strcpy(macbuf, "??");
		p_debug("probe client %s (cookie %llx): %s\n",
		       macbuf,
		       (unsigned long long)nla_get_u64(tb[NL80211_ATTR_COOKIE]),
		       tb[NL80211_ATTR_ACK] ? "acked" : "no ack");
		break;
	case NL80211_CMD_VENDOR:
		p_debug("vendor event %.6x:%d\n",
			nla_get_u32(tb[NL80211_ATTR_VENDOR_ID]),
			nla_get_u32(tb[NL80211_ATTR_VENDOR_SUBCMD]));
		break;
	default:
		p_debug("unknown event %d (%s)\n",
