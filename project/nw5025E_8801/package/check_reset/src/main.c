...
int get_wifi_connect_info()
{
	...
	memset( buffer, 0, 512 ); 
	read_fp = popen("mlanutl uap0 getstalist | grep \"MAC Address\"", "r");
	if(read_fp!=NULL){
	...
	memset( buffer, 0, 512 );
	read_fp = popen("route -n | grep mlan0 | awk '{print $2}' | grep -v \"0.0.0.0\"", "r");
	if(read_fp!=NULL){
		chars_read = fread(buffer, sizeof(char), 512-1, read_fp); 
		if (chars_read > 0)   	//connect !
		{ 
...
int main() 
{
	...
	int counter = 0;
	int pc_disable = 0;
	int pc_disable_now = 0;
	int pc_disable_old = 0;
	struct cmd_info cmd;
	int i = 0;
	int    socket_id;
	int power_sta=0;
	...
	ret = get_power_status(&power_sta);
	if(ret == 0 && power_sta == NOT_NORMAL_POWER)
	{
	...
	else
	{
		...
		else
		{
			...
			system("echo uap0 > /sys/class/leds/longsys\:green\:led/device_name");
	...
	while(1){	
		...
		{ //turn on the wifi
			if(get_wifi_mode()==M_2G)
			{
				...
				system("echo 0 > /sys/class/leds/longsys\:green\:led/brightness");
				system("echo uap0 > /sys/class/leds/longsys\:blue\:led/trigger");
		...
		if(detect_status_now != DETECT_PC && reset_status == BUTTON_DOWN_LED_OFF){	
			...
			system("sh /usr/mips/cgi-bin/script/start_smartlink.sh &");
		}

		ret = get_wifi_connect_info();
		if(ret == 0 && detect_status_now != DETECT_PC){
			idle_cnt++;
		}
		else{
			idle_cnt=0;
