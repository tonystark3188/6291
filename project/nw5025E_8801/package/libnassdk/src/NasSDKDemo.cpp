...
//char g_root[] = "/tmp/mnt/SD-disk-1";
static char initPath[] = "/tmp/mnt";
static char g_root[512];
static char	firstDir[512];
static int  g_isLogin = 1;    //0 表示已登录，1表示未登录


static const struct nas_file_type_dir default_nas_file_types_dir[] = {
	{file_type_pic, 	"My Pictures"},
	{file_type_doc, 	"My Documents"},
	{file_type_audio,	"My Music"},
	{file_type_video,	"My Videos"},
	{file_type_file, 	"Others"}
};
...
int relative2abspath(unsigned long long from_client, const char* relative_path, char* abs_path, int len)
{
	...
	printf("relative2abspath: relative_path = %s  ,  abs_path = %s",relative_path, abs_path);
	return 0;
}
...
int notify_file_download_complete(unsigned long long from_client, const char* tmp_file_path, int file_type, const char* specified_dst_dir)
{
	...
	//int ret = 0;
	char dst_dir[512] = {0};
	memset(dst_dir, 0, 512);
	
	char *filename = strrchr(tmp_file_path,'/');
	//如果spcified_dst_dir 为NULL，用户可以通过file_type来自定义文件保存位置，这里为演示，统一放到当前目录
	if(specified_dst_dir == NULL){
		printf("specified_dst_dir is null\n");
		char format[16] = "\0";
		char typeDir[32] = "\0";
		int file_type;
		const struct nas_file_type_dir *td;
		memset(format, 0, sizeof(format));
		memset(typeDir, 0, sizeof(typeDir));
		
		get_file_ext(filename,format);
		file_type=get_file_type(format);
		
		for(td = default_nas_file_types_dir; td->dir_name != NULL; td++) {
			if(file_type == td->file_type){
				strcpy(typeDir, td->dir_name);
				break;
			}
		}

		//snprintf(dst_dir, 512, firstDir);
		snprintf(dst_dir,512,"%s/%s", firstDir, typeDir);
	}else{
		printf("specified_dst_dir: %s\n", specified_dst_dir);
		snprintf(dst_dir,512,"%s/%s", firstDir, specified_dst_dir);           //存放到specified_dst_dir目录下. specified_dst_dir为相对路径，需转换为绝对路径  
	}
	...
	printf(" copy src_file[%s] to dst_file[%s]",src_file, dst_file);	

	//复制或移动文件
	//DU_File::copyFile(src_file,dst_file,true);
	int ret = copyfile(src_file,dst_file);
	if(ret){
...
int process_datapoint_file_list(unsigned long long from_client, tx_data_point *req_data_point, tx_data_point* rsp_data_point)
{
	...
	relative2abspath(from_client,file_path,abspath,512);
	printf("abspath=%s\n",abspath);
...
void* thread_func_initdevice(void * arg)
{
    ...
    int nPubkeySize = 0;
    if (0 != readBufferFromFile("/etc/qq/1700001992.pem", svrPubkey, sizeof(svrPubkey), &nPubkeySize))
    {
	...
	sprintf(tmpPath, "%s/.tmp/", firstDir);

	//ret = tx_nas_init_file_transfer(&file_transfer_nofity, "/tmp/mnt/SD-disk-1/.tmp/");
	ret = tx_nas_init_file_transfer(&file_transfer_nofity, tmpPath);
	if (0 == ret) {
	...
	while((ent=readdir(pDir))!=NULL){
		if(ent->d_type & DT_DIR){
			if(strcmp(ent->d_name,".")==0 || strcmp(ent->d_name,"..")==0)  
				continue;
			if(!dirCount)
				sprintf(firstDir, "%s/%s", initPath, ent->d_name);
			dirCount++;
		}
	}
...
int main(int argc, char* argv[])
{
	...
	printf("initPath: %s, firstDir: %s, g_root: %s\n", initPath, firstDir, g_root);
