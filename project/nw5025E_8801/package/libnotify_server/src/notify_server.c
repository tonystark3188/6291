...
int FileParseGetDbwWorkStatusDataJson(ClientTheadInfo *p_clientInfo)
{
	int ret = 0;
	JObj *data_json = JSON_GET_OBJECT(p_clientInfo->r_json,"data");
	if(data_json == NULL){
		p_clientInfo->error = -10002;
		goto FAIL_EXIT;
	}
	...
	p_clientInfo->status = JSON_GET_OBJECT_VALUE(status_json,int);

	if(p_clientInfo->r_json != NULL)
		JSON_PUT_OBJECT(p_clientInfo->r_json);
	return 0;
FAIL_EXIT:
	if(p_clientInfo->r_json != NULL)
		JSON_PUT_OBJECT(p_clientInfo->r_json);
	return p_clientInfo->error;
}


int notify_comb_release_disk_send_buf(ClientTheadInfo *p_client_info)
{
    int res_sz = 0;
	...
	JSON_ADD_OBJECT(para_info, "event",JSON_NEW_OBJECT(p_client_info->event,int));
...
int udisk_extract_notify_comb_release_disk_send_buf(ClientTheadInfo *p_client_info)
{
    int res_sz = 0;
    JObj* response_json = JSON_NEW_EMPTY_OBJECT();
	JObj* header_json=JSON_NEW_EMPTY_OBJECT();
	...
	JSON_ADD_OBJECT(para_info, "release_flag",JSON_NEW_OBJECT(p_client_info->release_flag,int));
	JSON_ADD_OBJECT(para_info, "event",JSON_NEW_OBJECT(p_client_info->event,int));
	JSON_ADD_OBJECT(para_info, "action_node",JSON_NEW_OBJECT(p_client_info->action_node,string));
	JSON_ARRAY_ADD_OBJECT (response_data_array,para_info);
	JSON_ADD_OBJECT(response_json, "data", response_data_array);
	JSON_ADD_OBJECT(header_json, "cmd", JSON_NEW_OBJECT(p_client_info->cmd,int));
	JSON_ADD_OBJECT(header_json, "seq", JSON_NEW_OBJECT(p_client_info->seq,int));
...
int CombGetDbWorkStatusSendBuf(ClientTheadInfo *p_clientInfo)
{
    int res_sz = 0;
    JObj* response_json = JSON_NEW_EMPTY_OBJECT();
	JObj* header_json=JSON_NEW_EMPTY_OBJECT();
	...
	JSON_ADD_OBJECT(header_json, "cmd", JSON_NEW_OBJECT(p_clientInfo->cmd,int));
	JSON_ADD_OBJECT(header_json, "seq", JSON_NEW_OBJECT(p_clientInfo->seq,int));
	JSON_ADD_OBJECT(header_json, "error", JSON_NEW_OBJECT(p_clientInfo->error,int));
	JSON_ADD_OBJECT(response_json, "header", header_json);
	char *response_str = JSON_TO_STRING(response_json);
	if(response_str == NULL){
		JSON_PUT_OBJECT(response_json);
	...
	res_sz = strlen(response_str);
	p_clientInfo->retstr = (char*)malloc(res_sz + 1);
	if(p_clientInfo->retstr == NULL){
		JSON_PUT_OBJECT(response_json);
		return -1;
	}	
	
	strcpy(p_clientInfo->retstr,response_str);
    DMCLOG_D("send_buf = %s", p_clientInfo->retstr);
	JSON_PUT_OBJECT(response_json);
	return 0;
}

//pc mount event notify server
int notify_server_release_disk(int release_flag,int *p_status)
{
	...
	client_info.event = EVENT_PC_MOUNT;
	ret = notify_comb_release_disk_send_buf(&client_info);
	if(ret < 0){
		DMCLOG_E("combile error");
		goto _HANDLE_FAILED;
	}
    ...
    if(client_info.client_fd > 0){
		...
		if(ret != 0){
            DMCLOG_E("parser data json error");
			//ret = client_info.error;
            goto _HANDLE_FAILED;
        }
...
//udisk extract event notify server
int udisk_extract_notify_server_release_disk(int release_flag, char *action_node, int *p_status)
{
	...
	client_info.release_flag = release_flag;
	if(action_node == NULL){
		DMCLOG_E("cation node is null");
		goto _HANDLE_FAILED;
	}
	strcpy(client_info.action_node, action_node);
	client_info.event = EVENT_UDISK_EXTARCT;
	ret = udisk_extract_notify_comb_release_disk_send_buf(&client_info);
	if(ret < 0){
		DMCLOG_E("combile error");
		goto _HANDLE_FAILED;
	}
...
int GetDbWorkStatus(int *p_status)
{
	...
	memset(&client_info,0,sizeof(ClientTheadInfo));
    client_info.time_out = RECV_TIMEOUT;
	client_info.cmd = FN_GET_DB_STA;
	strcpy(client_info.ip,LOCAL_ADDR);
	ret = CombGetDbWorkStatusSendBuf(&client_info);
	if(ret < 0){
		DMCLOG_E("combile error");
		goto _HANDLE_FAILED;
	}
    ...
    if(client_info.client_fd > 0){
		...
		ret = FileParseGetDbwWorkStatusDataJson(&client_info);
		if(ret != 0){
            DMCLOG_E("parser data json error");
			//ret = client_info.error;
            goto _HANDLE_FAILED;
...
//TODO
	...
	//system("touch /tmp/notify_log");
	snprintf(str,sizeof(str),"echo \"action=%d,filepath=%s\" >> /tmp/notify_log",action,filepath);
	system(str);
#endif
}
