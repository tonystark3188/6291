	...
	while( (ent=readdir(pDir))!=NULL )
	{
		...
		if( S_ISDIR(statbuf.st_mode) )  //is a dir
		{
			...
			if( (fw_fp=fopen(fw_path,"rb")) != NULL )
			{
				...
				sprintf(cmd,"cp %s/'$$update$$.bin' /tmp/fwupgrade",dir);
				system(cmd);
	...
	while( (ent=readdir(pDir))!=NULL )
	{
		...
		if( S_ISDIR(statbuf.st_mode) )  //is a dir
		{
			if(strcmp( ".",ent->d_name) == 0 || strcmp( "..",ent->d_name) == 0) 
				continue;
			memset(fw_path,0,sizeof(fw_path));
			sprintf(fw_path,"%s/$$reset$$.bin",dir);
			if( (fw_fp=fopen(fw_path,"rb")) != NULL )
			{
				fclose(fw_fp);
				system("sync");
				sprintf(cmd,"rm -f %s/'$$reset$$.bin'",dir);
				system(cmd);
				system("sync");
				system("jffs2reset -y");
...
int main(int argc, char const *argv[])
{
	...
	//execute reset
	printf("check reset file from udisk\n");
	check_reset_file();
