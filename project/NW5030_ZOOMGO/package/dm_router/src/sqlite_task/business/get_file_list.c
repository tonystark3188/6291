...
{ 
	...
	else if(c->cmd == 124 || c->cmd == 101)
	{
        ...
        DMCLOG_D("get_db_opr_obj(c->msg).data.file_list.path = %s",get_db_opr_obj(c->msg).data.file_list.path);
	}
    ...
    DMCLOG_D("get file list lock in");
    ...
    DMCLOG_D("get file list lock out");
	pthread_mutex_unlock(&c->disk_info->mutex);
...
int handle_encrypt_file_insert_cmd(char *file_uuid,char *src_path,char bIsRegularFile,char *uuid,char *encryptPath,long date,ssize_t encryFileSize)
{
    Message *msg;
    ...
    if(bIsRegularFile)
    {
        ...
        get_db_opr_obj(msg).data.insert_data.file_info.encrypt_info.fileSize = encryFileSize;
        get_db_opr_obj(msg).data.insert_data.file_info.encrypt_info.date = date;
...
int handle_encrypt_file_query_cmd(_In_ char *uuid,_In_ char *filePath,_Out_ ssize_t *fileSize,_Out_ long *date,_Out_ char *encryptPath)
{   
    Message *msg;
	int ret = 0;
    // 2. query db.
    ...
    *date = file_info->encrypt_info.date;
    ...
    strcpy(encryptPath,file_info->encrypt_info.path);
