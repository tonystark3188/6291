...
static error_t update_file_info_by_id(sqlite3 *database, uint32_t id,file_info_t *pfi)
{
	...
	sprintf(sql,"UPDATE %s SET FILE_UUID='%s', CHANGE_TIME=%u,TYPE=%d,SIZE=%lld WHERE ID=%u", FILE_TABLE_NAME, pfi->file_uuid, pfi->modify_time,pfi->file_type,pfi->file_size,id);
...
void load_file_item(char **FieldName, char **FieldValue, int nFields, file_info_t *pfi)
{
    ...
    if(FieldValue[13] != NULL)
    {
        ...
        if(pfi->encryptId != 0)
            DMCLOG_E("pfi->encryptId = %d",pfi->encryptId);
    }
...
static error_t file_table_delete_normal(sqlite3 *database,char *path)
{
	...
	if(pfi.isFolder == 0)//delete normal file
	{
		...
		errcode = dm_do_delete(database,pfi.index);
		if(errcode!= RET_SUCCESS)
		{
			return errcode;
		}
    ...
    if(errcode == EDB_RECORD_NOT_EXIST)
   	{
        ...
        file_tmp.path = pfi->path;
        file_tmp.name = bb_basename(pfi->path);
        file_tmp.file_type = pfi->file_type;
        ...
        DMCLOG_D("file :%s is exist 1.0.1",pfi->path);
        update_file_info_by_id(database,id,pfi);
    }
    return errcode;
}
