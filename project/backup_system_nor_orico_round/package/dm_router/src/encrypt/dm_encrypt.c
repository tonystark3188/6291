...
void _dm_gen_uuid(_Out_ char* uuid, _In_ const char *pid, _In_ const char *vid, _In_ unsigned long long total_size,_In_ unsigned long long free_size)
{
	char src[g_len_encrypt_src]	= {0};
	char buffer[36]		= {0};

	int	noise_start_total_size;
	int	noise_start_free_size;
	int	i;
	...
	/* 2, 根据时间取干扰串起始位置 */
	noise_start_total_size = total_size % g_len_encrypt_src;
	noise_start_free_size = free_size % g_len_encrypt_src;
	DMCLOG_D("total_size22 = %llu, noise_start_total_size = %d, noise_start_free_size = %d",total_size, noise_start_total_size, noise_start_free_size);

	/* 3, 用干扰串异或 */
	//for (i=noise_start; i<g_len_encrypt_src +noise_start; ++i)
	for (i=0; i<g_len_encrypt_src; ++i)
	{
		if(i < g_len_encrypt_src/2){
			if (g_len_encrypt_src > i + noise_start_total_size)
			{
				src[i]	= src[i] ^ g_enc_noise[i+noise_start_total_size];
			}
			else
			{
				src[i]	= src[i] ^ g_enc_noise[i+noise_start_total_size-g_len_encrypt_src];
			}
		}
		else{
			if (g_len_encrypt_src > i + noise_start_free_size)
			{
				src[i]	= src[i] ^ g_enc_noise[i+noise_start_free_size];
			}
			else
			{
				src[i]	= src[i] ^ g_enc_noise[i+noise_start_free_size-g_len_encrypt_src];
			}
		}
	}
