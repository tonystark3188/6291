...
int getSerialInfo(xmlNodePtr tag, char *retstr);
...
tagHandle gtaghandle_get[]=
{
	...
	{FN_GET_SERIAL_INFO,getSerialInfo},
};
...
int getSerialInfo(xmlNodePtr tag, char *retstr)
{
	...
	if(!get_cfg_str("serial_no", serialNo)){
		sprintf(retstr,"<%s %s=\"\" </%s>",FN_GET_SERIAL_INFO, GET_SERIAL_NO, FN_GET_SERIAL_INFO);
	}
	else{		
		sprintf(retstr,"<%s %s=\"%s\" </%s>",FN_GET_SERIAL_INFO, GET_SERIAL_NO, serialNo, FN_GET_SERIAL_INFO);
	}
