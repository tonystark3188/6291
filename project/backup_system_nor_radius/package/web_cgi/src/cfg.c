...
#define SERIAL_NO_ADDR			(CFG_BASE_ADDR+DATA_LEN*36)
#define SERIAL_NO_LEN			64	//32*2
...
static void usage(void)
{
	...
	printf("	host_name version_flag repair_auto qqsn license root_pwd serial_no\n\n");
...
int cfg_get(char *argv[])
{
	...
	else if(!strcmp(argv[2],SERIAL_NO))
	{
		unsigned char tmp[32];
		memset(tmp,0,sizeof(tmp));
		read_mtd(tmp,fp_mtd,SERIAL_NO_ADDR,SERIAL_NO_LEN);
		if(strstr(tmp,SERIAL_NO)!=NULL)
		{
			printf("%s",(char *)tmp);
			printf("\n");
		}
...
//--------------------------------------------------------------------
		...
		memset(change_etc_pwd_str,0,sizeof(change_etc_pwd_str));
		sprintf(change_etc_pwd_str,"sed -e \'s/^%s:/%s:/g\' -i /etc/samba/smbpasswd","radius_RW-WPS",argv[2]+strlen(SMB_USR_NAMR)+1);
		system(change_etc_pwd_str);
		memset(change_etc_pwd_str,0,sizeof(change_etc_pwd_str));
		sprintf(change_etc_pwd_str,"sed -e \'s/:%s:/:%s:/g\' -i /etc/samba/smbpasswd","radius_RW-WPS",argv[2]+strlen(SMB_USR_NAMR)+1);
		system(change_etc_pwd_str);
//-----------------------------------------------------------------
		memset(change_etc_pwd_str,0,sizeof(change_etc_pwd_str));
		sprintf(change_etc_pwd_str,"sed -e \'s/^%s:/%s:/g\' -i /etc/passwd","radius_RW-WPS",argv[2]+strlen(SMB_USR_NAMR)+1);
		DEBUG(printf("change /etc/passwd : %s",change_etc_pwd_str));
		system(change_etc_pwd_str);
		memset(change_etc_pwd_str,0,sizeof(change_etc_pwd_str));
		sprintf(change_etc_pwd_str,"sed -e \'s/:%s:/:%s:/g\' -i /etc/passwd","radius_RW-WPS",argv[2]+strlen(SMB_USR_NAMR)+1);
...
//-----------set nor param---------------------------------------------------
	...
	else if(!strncmp(SERIAL_NO,argv[2],strlen(SERIAL_NO)))
	{
		strcpy(data_str,argv[2]);
		write_mtd(data_str,fp_mtd,SERIAL_NO_ADDR,strlen(data_str)+1);
	}
	else
	{
		#if Debug_cfg
