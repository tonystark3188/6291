...
int main() 
{
	...
	if(power_sta == NORMAL_POWER )
	{
		...
		if(get_wifi_mode()==M_2G)
		{
			...
			system("echo timer > /sys/class/leds/longsys\:blue\:led/trigger");
			system("echo 2000 > /sys/class/leds/longsys\:blue\:led/delay_on");
		...
		else
		{
			...
			system("echo 2000 > /sys/class/leds/longsys\:green\:led/delay_on");
			system("echo 1000 > /sys/class/leds/longsys\:green\:led/delay_off");
		}
