...
/*
  *获取文件系统信息
  */
int rfsvfs_statvfs(const char *path, struct statvfs *buf)
{
	char all_path[RFSVFS_ALL_PATH_LENGTH_DEFAULT]={0};
	get_all_file_path(all_path,path);
	printf("file_path is %s\n",all_path);
	return (statvfs(all_path, buf));
}

/*
  *设置文件时间
  */
int rfsvfs_utimensat(int dirfd, const char *pathname, 
		const struct timespec times[2], int flags)
{
	char all_path[RFSVFS_ALL_PATH_LENGTH_DEFAULT]={0};
	get_all_file_path(all_path,pathname);
	printf("file_path is %s\n",all_path);	
	return (utimensat(dirfd, all_path, times, flags));
}
