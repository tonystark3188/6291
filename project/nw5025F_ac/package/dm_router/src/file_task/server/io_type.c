...
all_read_type(struct stream *stream, void *buf, size_t len)
{
    ...
    DMCLOG_D("c->nfiles = %d,count = %d",c->nfiles,get_db_opr_obj(c->msg).data.file_list.result_cnt);
	if(c->nfiles == get_db_opr_obj(c->msg).data.file_list.result_cnt)
	{
        ...
        DMCLOG_D("c->cmd = %d,c->nfiles = %d,count = %d",c->cmd,c->nfiles,get_db_opr_obj(c->msg).data.file_list.result_cnt);
        ...
        if(c->nfiles < MAX_FILE_NUM_LEN)//finished
        {
            ...
            //TODO: need to add token to get file by type.
			//if(strcmp(c->token,"") == 0)
				c->status = 1;//do not have token,filter encrypt file
			//else 
...
//	DMCLOG_D("drive_count = %d",mAll_disk_t.count);
	...
	for(i = 0;i < mAll_disk_t.count;i++)
	{
		...
		if(res != RET_SUCCESS)
		{
			DMCLOG_D("handle_get_all_type_file_list_cmd error");
			c->error = ERROR_GET_FILE_LIST;
			c->loc.flags |= FLAG_CLOSED;
			return;
		}
