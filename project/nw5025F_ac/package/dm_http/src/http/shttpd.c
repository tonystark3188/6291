...
ppc_get_path_info(struct conn *c, char *path, struct stat *stp)
{
	char	*p, *e;
	#ifdef _PPCLIB_H_
	if (ppc_stat(path, stp, c->token) == 0)
		return (0);
	#else
	if (my_stat(path, stp) == 0)
		return (0);
	...
	for (; p > e; p--)
		if (*p == '/') {
			*p = '\0';
			if (!ppc_stat(path, stp, c->token) && !S_ISDIR(stp->st_mode)) {
				c->path_info = p + 1;
				return (0);
			} else {
				*p = '/';
...
#if 0
int check_signature_form_url(char *url){
	int ret = 0;
	char *access_key = NULL;
	char *ssig = NULL;

	if((access_key = get_arg_from_url(url, ACCESS_KEY_NAME)) == NULL){
		DMCLOG_E("get arg:%s fail", ACCESS_KEY_NAME);
		return -1;
	}

	if((ssig = get_arg_from_url(url, SSIG_NAME)) == NULL){
		DMCLOG_E("get arg:%s fail", ACCESS_KEY_NAME);
		return -1;
	}

	// TODO: 输入字符串应该是什么
	ret = check_signature(url, access_key, ssig, MATCH_SSIG_START, MATCH_SSIG_END);
	if(!ret){
...
decide_what_to_do(struct conn *c)
{
	...
	if ((c->query = strchr(c->uri, '?')) != NULL){
		*c->query++ = '\0';
		...
		if((sign_url = (char *)calloc(1, c->query - c->uri)) != NULL){
			memcpy(sign_url, c->uri, c->query - c->uri - 1);
			DMCLOG_D("sign_url: %s", sign_url);
			if((access_key = get_arg_from_url(c->query, ACCESS_KEY_NAME)) != NULL){
	...
	char uri_buf[16];
	memset(uri_buf, 0, 16);
	memcpy(uri_buf, c->uri, 15);
	if(NULL != strstr(c->uri, WEBDAV_NAME)){
		char *p_path = strstr(c->uri, WEBDAV_NAME);
		p_path = p_path + sizeof(WEBDAV_NAME) - 1;
		//(void) my_snprintf(path, sizeof(path), "%s%s",
	    //	MOUNT_PATH, p_path);
	    (void) my_snprintf(path, sizeof(path), "%s", p_path);
	}
	else if((NULL != strstr(c->uri, USB_DISK_NAME)) || (NULL != strstr(c->uri, SD_DISK_NAME)) || (NULL != strstr(c->uri, PRIVATE_DISK_NAME))){
		(void) my_snprintf(path, sizeof(path), "%s%s",
    		MOUNT_PATH, c->uri);
	}
	else{
	...
	if(is_fw_filesystem)
	{
		...
		if (c->method == METHOD_PUT) {
			...
			//printf("path_buf = %s\n",path_buf);
			if (((c->loc.chan.dir.path = my_strdup(path)) != NULL) && ((c->loc.chan.fd = my_open(path_buf,
				...
				if(NULL != c->loc.chan.dir.path){
					free(c->loc.chan.dir.path);
				}
			...
			DMCLOG_D("path = %s,cgi_extensions = %s",path,c->ctx->cgi_extensions);
	...
	else
	{
		...
		if (c->method == METHOD_PUT) {
			...
			if (c->ch.range.v_vec.len > 0) {
				send_server_error(c, 501, "PUT Range Not Implemented");
		#ifdef _PPCLIB_H_			
			} else if ((rc = ppc_put_dir(c, path)) == 0) {
		#else
			} else if ((rc = put_dir(c, path)) == 0) {
		#endif
				send_server_error(c, 200, "OK");
...
disconnect(struct conn *c)
{
	...
	if (c->uri)
	{
		...
		memset(uri_buf, 0, 16);
		memcpy(uri_buf, c->uri, 15);
		if(strstr(uri_buf,WEBDAV_NAME)&&c->uri[strlen(c->uri) - 1] == '/')
		{	
			DBG(("get header shtml finish uri = %s\n",c->uri));
			#ifdef _PPCLIB_H_
			if ((c->loc.chan.dir.dirp = ppc_opendir(c->loc.chan.dir.path, c->token)) == NULL) {
			#else
	...
	if (dont_close) {
		...
		c->query = c->request = c->path_info = NULL;
		c->mime_type = NULL;
		...
		int uri_len = 0;
		if(NULL != c->uri){
			uri_len = strlen(c->uri);
			parent_uri = (char *)malloc(uri_len+1);
			if(NULL != parent_uri){				
				memset(parent_uri, 0, uri_len+1);
				strcpy(parent_uri, c->uri);
				p_uri = strrchr(parent_uri, '/');
				if(NULL != p_uri){
					memset(p_uri, 0, strlen(p_uri));
					p_uri = strrchr(parent_uri, '/')+1;
					if(NULL != p_uri)
						memset(p_uri, 0, strlen(p_uri));
				}
				DBG(("parent_uri = %s\n", parent_uri));
				c->loc.io.head = my_snprintf(c->loc.io.buf, c->loc.io.size,
		    "<table><tr><th><img src=\"/icons/48x48/blank.png\" alt=\"[ICO]\"></th>\
<th><a href=\"?C=N;O=D\">Name</a></th><th><a href=\"?C=M;O=A\">Last modified</a></th><th><a href=\"?C=S;O=A\">Size</a></th></tr><tr><th colspan=\"4\"><hr></th></tr>\
<tr><td valign=\"top\"><a href=\"%s\"><img src=\"/icons/48x48/folder-parent.png\" alt=\"[DIR]\"></a></td>\
<td><a href=\"%s\">Parent Directory</a></td><td>&nbsp;</td><td align=\"right\">  - </td></tr>", parent_uri, parent_uri);
				free(parent_uri);
			}
			else{
				DBG(("parent_uri is null\n"));
				c->loc.io.head = my_snprintf(c->loc.io.buf, c->loc.io.size,
		    "<table><tr><th><img src=\"/icons/48x48/blank.png\" alt=\"[ICO]\"></th>\
<th><a href=\"?C=N;O=D\">Name</a></th><th><a href=\"?C=M;O=A\">Last modified</a></th><th><a href=\"?C=S;O=A\">Size</a></th></tr><tr><th colspan=\"4\"><hr></th></tr>\
<tr><td valign=\"top\"><a href=\"/webdav/\"><img src=\"/icons/48x48/folder-parent.png\" alt=\"[DIR]\"></a></td>\
<td><a href=\"/webdav/\">Parent Directory</a></td><td>&nbsp;</td><td align=\"right\">  - </td></tr>");
			}	
		}
		else{
			DBG(("c->uri is null\n"));
...
shttpd_poll(struct shttpd_ctx *ctx, int milliseconds)
{
	...
	LL_FOREACH_SAFE(&ctx->connections, lp, tmp) {
		...
		//DBG(("loc: %u [%.*s]", io_data_len(&c->loc.io),io_data_len(&c->loc.io), io_data(&c->loc.io)));
		//DBG(("rem: %u [%.*s]", io_data_len(&c->rem.io),io_data_len(&c->rem.io), io_data(&c->rem.io)));
		...
		if (io_space_len(&c->loc.io) &&
		    ...
		    //DBG(("read_stream loc, set = %d, fd = %d\n", FD_ISSET(c->loc.chan.fd, &read_set), c->loc.chan.fd));
			read_stream(&c->loc);
		}
