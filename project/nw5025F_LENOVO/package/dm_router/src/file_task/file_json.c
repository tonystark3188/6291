...
file_tag_handle all_file_handle[]=
{
	...
	{FN_ROUTER_GET_P2P_KEY, _dm_get_p2p_key, Parser_RouterSetSafeExit},
...
int del_file_inotify(const char *path,del_info_t *pInfo)
{
	...
	{
		pInfo->status = 1;
		pInfo->cur_name = (char *)calloc(1,strlen(bb_basename(path)) + 1);
	    strcpy(pInfo->cur_name,bb_basename(path));
...
int copy_file_inotify(const char *source, const char *dest,copy_info_t *pInfo)
{
    ...
    if(S_ISREG(source_stat.st_mode))
    {
		...
		//DMCLOG_D("src_fd = %d,dst_fd = %d",src_fd,dst_fd);
...
int Parser_FileGetListByType(struct conn *c)
{
	...
	if(offset_json != NULL)
	{
		c->off = JSON_GET_OBJECT_VALUE(offset_json,int);
	}
	JObj *length_json = JSON_GET_OBJECT(para_json,"length");
	if(length_json != NULL)
	{
		c->len = JSON_GET_OBJECT_VALUE(length_json,int);
	}
