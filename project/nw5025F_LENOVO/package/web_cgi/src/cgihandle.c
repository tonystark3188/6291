...
int getAddNetwork(xmlNodePtr tag, char *retstr);
...
tagHandle gtaghandle_set[]=
{
	...
	{FN_Upgrade,Upgrade},
	{FN_HALT,halt},
	{FN_SYNCSYS,SyncSystem},
	{FN_Time_Sync,TimeSync},
	{FN_SET_Client,setClient},
	{FN_SET_3G,set3G},
	{FN_SET_iperf,setiperf},
	{FN_SET_OTAStatus,setotastatus},
	{FN_SET_FsckTest,setFsckTest},
	{FN_SET_RepairHandle,setRepairHandle},
	{FN_SET_RepairAuto,setRepairAuto},
	{FN_SET_ForgetWifiInfo,setForgetWifiInfo},
	{FN_SET_DisabledClient,setDisabledClient},
	{FN_SET_SetDMClient,setDMClient},
};
...
int getssid(xmlNodePtr tag, char *retstr)
{
	...
	wifi_module = get_wifi_module();
	if(wifi_module == AP6255)
		strcpy(ifname,"wlan0");
	else if(wifi_module == MRVL8801)
		strcpy(ifname,"mlan0");
...
int internet(xmlNodePtr tag, char *retstr)
{
	...
	sprintf(retstr,"<internet is_net_ok=\"%d\"></internet>",is_net_ok);
	return 0;
}
...
int getdiskotaupgrade(xmlNodePtr tag, char *retstr)
{
    //char geturl[256] = {0};
    char version_flag[32] = "\0";
    char *customcode=CUSCODE;
    char mac[32]="\0";
	char tmp_mac[32]="\0";
   // char uci_option_str[128]="\0";
   // char webreturn[OTASIZE] = {0};
	...
	get_cfg_str("fw_version",prj_num);


	if( (read_fp=fopen("/etc/mac.txt", "rb")) != NULL)
	{
		fread(tmp_mac,1,17,read_fp);
		strncpy(mac,tmp_mac,17);
		fclose(read_fp);
	}

   	#if 0
	//http://x.dmsys.com:8017/upgrade/?customCode=41&versionCode=1110&mac=FF:FF:FF:FF:FF:FF&language=en
    sprintf(geturl,"wget --no-check-certificate https://x.dmsys.com/GetXml?customCode=%s\\&versionCode=%s\\&mac=%s\\&language=%s -O /tmp/otaupgradefile",customcode,version,mac,langue);
    system(geturl);
	//cgi_log(geturl);
    fp = fopen("/tmp/otaupgradefile","r");
    if(NULL == fp)
    {
        strcpy(retstr,"<OTAupgrade><Newfw status=\"SUPPORTED\">{\"status\":\"nothing\"}</Newfw></OTAupgrade>");
        return 1;
    }
    //memset(retstr,0x0,OTASIZE);
    fread(webreturn,OTASIZE-1,1,fp);
	if(strlen(webreturn) == 0)
	{
		strcpy(retstr,"<OTAupgrade><Newfw status=\"SUPPORTED\">{\"status\":\"nothing\"}</Newfw></OTAupgrade>");
        return 1;	
	}
    sprintf(retstr,"<OTAupgrade><Newfw status=\"SUPPORTED\">%s</Newfw></OTAupgrade>",webreturn);
 
    
    fclose(fp);
    system("rm -f /tmp/otaupgradefile &");
	#endif
	
	if( (cfg_ret == 0) || (strcmp(version_flag,"1") == 0))
	{
		sprintf(retstr,"<GetOTAInfo customCode=\"%s\" versionCode=\"%s\" mac=\"%s\" versionflag=\"1\" time=\"90\"></GetOTAInfo>",prj_num,fw_ver_num_tmp,mac);
	}
	else
	{
		sprintf(retstr,"<GetOTAInfo customCode=\"%s\" versionCode=\"%s\" mac=\"%s\" versionflag=\"0\" time=\"90\"></GetOTAInfo>",prj_num,fw_ver_num_tmp,mac);
	}
...
int getRepairAuto(xmlNodePtr tag, char *retstr)
{
	char repair_auto_enable[32];
	int cfg_ret = 0;
	int ret = 0;
	
	memset(repair_auto_enable, 0, 32);
	cfg_ret = get_cfg_str("repair_auto", repair_auto_enable);
	if((cfg_ret == 0) || (0 == strcmp(repair_auto_enable, "on")))
	{
		sprintf(retstr,"<RepairAuto enable=\"on\"></RepairAuto>");
	}
	else
	{
		sprintf(retstr,"<RepairAuto enable=\"off\"></RepairAuto>");
	}
...
int getAddNetwork(xmlNodePtr tag, char *retstr)
{	
	sprintf(retstr,"<AddNetwork enable=\"ON\"></AddNetwork>");
	return 0;
}
...
int SyncSystem(xmlNodePtr tag, char *retstr)
{
	...
	if((pxml=xmlGetprop(tag,(const xmlChar*)Sync_Clean_Cache))!=NULL)
	{
		if((value=(char *)malloc(strlen(pxml)+1))!=NULL)
		{
			memset(value,0,strlen(pxml)+1);
...
int setClient(xmlNodePtr tag, char *retstr)
{
	...
	if((pxml=xmlGetprop(tag,(const xmlChar*)SET_Client_enable))!=NULL)
	{
		...
		if(!strcmp(enable,"ON"))
		{
			...
			// {
			// 	sprintf(cmd_line,"ifconfig %s up",ifname);
			// 	system(cmd_line);
		...
		else if(!strcmp(enable,"OFF"))
		{
			...
			// sprintf(cmd_line,"ifconfig %s down",ifname);
			// system(cmd_line);
			// memset(cmd_line,0,sizeof(cmd_line));
		...
		sprintf(str_sp,"nor set client_disabled=%d",enable_config);
		system(str_sp);
		memset(str_sp,0,64);
...
int setFsckTest(xmlNodePtr tag, char *retstr)
{
	char *disk_power=NULL;
	char *pxml=NULL;

	if((pxml=xmlGetprop(tag,(const xmlChar*)SET_FsckTest_disk_power))!=NULL)
	{
		if((disk_power=(char *)malloc(strlen(pxml)+1))!=NULL)
		{
			memset(disk_power,0,strlen(pxml)+1);
			strcpy(disk_power,pxml);
			free(pxml);

			pxml=NULL;
		}

		if(!strcmp(disk_power,"0"))
		{
			system("stm8_control 2");
		}
		else
		{
			system("stm8_control 3");
		}
		free(disk_power);
	}
...
int setRepairHandle(xmlNodePtr tag, char *retstr)
{
	...
	if((pxml=xmlGetprop(tag,(const xmlChar*)SET_RepairHandle_mount_dir))!=NULL)
	{
		if((mount_dir=(char *)malloc(strlen(pxml)+1))!=NULL)
		{
			memset(mount_dir,0,strlen(pxml)+1);
			strcpy(mount_dir,pxml);
			free(pxml);
		...
		{
			memset(cmd, 0 ,32);
			sprintf(cmd, "handle_repair %s &", mount_dir);
			system(cmd);
		}
...
int setRepairAuto(xmlNodePtr tag, char *retstr)
{
	char *repair_auto_enable=NULL;
	char *pxml=NULL;
	char cmd[32];
	
	if((pxml=xmlGetprop(tag,(const xmlChar*)SET_RepairAuto_enable))!=NULL)
	{
		if((repair_auto_enable=(char *)malloc(strlen(pxml)+1))!=NULL)
		{
			memset(repair_auto_enable,0,strlen(pxml)+1);
			strcpy(repair_auto_enable,pxml);
			free(pxml);

			pxml=NULL;
		}
		
		if(NULL != repair_auto_enable)
		{
			if(!strcmp(repair_auto_enable, "on"))
			{
				system("cfg set repair_auto=on");
			}else if(!strcmp(repair_auto_enable, "off"))
				{
					system("cfg set repair_auto=off");
				}
		}

		free(repair_auto_enable);
	}
...
int setForgetWifiInfo(xmlNodePtr tag, char *retstr)
{
	...
	if((pxml=xmlGetprop(tag,(const xmlChar*)SET_ForgetWifiInfo_BSSID))!=NULL)
	{
		if((pbssid=(char *)malloc(strlen(pxml)+1))!=NULL)
		{
			memset(pbssid,0,strlen(pxml)+1);
			strcpy(pbssid,pxml);
			free(pxml);
		}
		else
...
ForgetWifiInfoexit:
	...
	else
	{
		printf("Content-type:text/html\r\n\r\n");
		printf("<?xml version=\"1.0\" encoding=\"utf-8\"?>");
		printf("<%s><Return status=\"false\" ></Return></%s>\r\n",SETSYSSTR,SETSYSSTR);
		fflush(stdout);
	}
	return 0;
}
...
int setDisabledClient(xmlNodePtr tag, char *retstr)
{
	...
	printstr("enter setDisabledClient");


	if((pxml=xmlGetprop(tag,(const xmlChar*)FN_SET_DisabledClient_status))!=NULL)
	{
		if((pstatus=(char *)malloc(strlen(pxml)+1))!=NULL)
		{
			memset(pstatus,0,strlen(pxml)+1);
			strcpy(pstatus,pxml);
			free(pxml);
		}
		else
...
int setDMClient(xmlNodePtr tag, char *retstr)
{
	...
	printstr("enter setDMClient");

	
	memset(uci_option_str,'\0',sizeof(uci_option_str));
	memset(port,'\0',sizeof(port));
	strcpy(uci_option_str,"dhcp.@dnsmasq[0].port");  
	uci_get_option_value(uci_option_str,port);
	memset(uci_option_str,'\0',sizeof(uci_option_str));
	memset(lanip,'\0',sizeof(lanip));
	strcpy(uci_option_str,"network.lan.ipaddr");  
	uci_get_option_value(uci_option_str,lanip);
	if((pxml=xmlGetprop(tag,(const xmlChar*)FN_SET_SetDMClient_Disabled))!=NULL)
	{
		if((pstatus=(char *)malloc(strlen(pxml)+1))!=NULL)
		{
			memset(pstatus,0,strlen(pxml)+1);
		...
		{
			free(pxml);
			goto setDMClientexit;
		}
		if(strcmp(pstatus,"1") == 0)
		{
			system("uci set wireless.@wifi-iface[1].disabled=1");
			system("uci commit");
			system("nor set client_disabled=1");
			system("control_dns.sh >/dev/null 2>&1 &");
		}
		else if(strcmp(pstatus,"0") == 0)
		{
			system("uci set wireless.@wifi-iface[1].disabled=0");
			system("uci commit");
			system("nor set client_disabled=0");
