...
int get_wifi_module()
{
	...
	if(strcmp(module,"AP6255")==0)
		return AP6255;
	else if(strcmp(module,"MRVL8801")==0)
		return MRVL8801;
}
...
void formatScanStr(char *outstr)
{
	...
	if(!strlen(ssidstr))  //remove the empty ssid
		return;
	// logstr(ssidstr);
	if(wifimode==M_2G && i_channel >14)
		return;
	if(wifimode==M_5G && i_channel <36)
		return;
	...
	if(is_connected_ap(ssidstr, record_password)){
...
{
	int ret;
	int wifi_module = get_wifi_module();
	int skfd;
