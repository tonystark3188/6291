...
int getMac(){
	FILE *read_fp = NULL;
	//read_fp = popen("cfg get mac | awk -F \"=\" '{print $2}' ", "r");
	read_fp = popen("ifconfig wlan0 |grep HWaddr |awk -F \" \" '{print $5}'","r");
	if(read_fp != NULL)
	{
		memset(tmp_mac,0,32);
...
void main()
{
		...
		char agent[256]={0};
		char osVer[32]={0};
		char phoneModel[256]={0};
		//Dalvik/2.1.0 (Linux; U; Android 6.0.1; MI 5s MIUI/V8.0.18.0.MAGCNDI)
		//LetvIphoneClient/6.12 (iPhone; iOS 10.2; Scale/2.00)
		strncpy(agent,getenv("HTTP_USER_AGENT"),256);
		p_debug("agent=%s",agent);
		char *target=strstr(agent,"Android");
		if(target != NULL){		
			char *p=strstr(target,";");
			//p_debug("p=%s",p);
			//p_debug("target=%s",target);
			if(p != NULL) {
				strncpy(osVer,target+strlen("Android")+1,p-target-strlen("Android")-1);
				//p_debug("osVer=%s",osVer);
				updateSysVal("osVer",osVer);
			}
			updateSysVal("osCode","10000");
		}
		target=strstr(agent,"iOS");
		if(target != NULL){		
			char *p=strstr(target,";");
			//p_debug("p=%s",p);
			//p_debug("target=%s",target);
			if(p != NULL) {
				strncpy(osVer,target+strlen("iOS")+1,p-target-strlen("iOS")-1);
				//p_debug("osVer=%s",osVer);
				updateSysVal("osVer",osVer);
			}
			updateSysVal("osCode","20000");
		}
		...
		updateSysVal("client_ssid",connected_ssid);
		...
		char tot[32]={0};
		char used[32]={0};
		sprintf(tot,"%lld",sto->total);
		sprintf(used,"%lld",sto->used);		
		updateSysVal("tot_s",tot);
		updateSysVal("used_s",used);
		...
		processString(web_str,"systemModel",phoneModel);
		updateSysVal("osModel",phoneModel);
		char *pssid=NULL;
		char *pkey=NULL;
		...
		//updateSysVal("version",version_info.curVersionName);
//		if(!strcmp(fw_sid,"")||(!strcmp(fw_sid,app_sid)&&(!strcmp(save_code,code))))//是管理员
		if(!strcmp(fw_sid,"")||(!strcmp(fw_sid,app_sid)))//是管理员
		{
			...
			system("uci commit");
			char cmd[256]={0};
			sprintf(cmd,"uci set userconfig.@save[0].sid=\"%s\" -c /factory/ && uci commit userconfig -c /factory",app_sid);
			system(cmd);
			...
			if(strcmp(save_code,code))//如果是管理员，随机码，则更新
				{
					...
					memset(uci_option_str,'\0',UCI_BUF_LEN);
					system("uci commit");
					sprintf(cmd,"uci set userconfig.@save[0].code=\"%s\" -c /factory/ && uci commit userconfig -c /factory",code);
					system(cmd);
			}
