...
void _exit_client(void *arg)
{
   ...
   // safe_free(p_client_info->retstr);
    p_debug("close client socket fd");
}
...
//	printf("int size=%d\n",sizeof(int));
	...
	sprintf(tmpStr,"ps -w |grep USB-disk");
//	printf("tmpStr=%s\n",tmpStr);
...
int notify_server()
{
	...
	strcpy(p_client_info.send_buf,buf);
    p_debug("send_buf = %s",p_client_info.send_buf);

    p_client_info.client_fd = DM_InetClientInit(AF_INET, TCP_INIT_PORT, SOCK_STREAM,p_client_info.ip);
    if(p_client_info.client_fd > 0)
    {
        if((ret = DM_MsgSend(p_client_info.client_fd,p_client_info.send_buf, strlen(p_client_info.send_buf))) != 0)
        {
		...
		//p_debug("p_client_info.recv_buf = %s",p_client_info.recv_buf);

		DM_DomainClientDeinit(p_client_info.client_fd);
		return ret;
		//sprintf(buf,"%s",p_client_info.recv_buf);
        /*ret = file_parse_process(p_client_info);
	        if(ret < 0)
	        {
	            p_debug("parser json error");
	            goto _HANDLE_FAILED;
	        }*/
    }
	else 
	    p_debug("Communication Init Error!");
_HANDLE_FAILED:
...
SINT32 DM_UdpSend(SINT32 iFd, const SINT8 *pBuf, UINT32 nLen,struct sockaddr_in *addr )
{
    ...
    if (pstMsg == NULL)
    {
        p_debug("malloc of msg header failed");
        return MSGRET_RESOURCE_EXCEEDED;
    }
    memset(pstMsg, 0, sizeof(MsgHeader));
    pstMsg->dataLength = nLen;
    ...
    if (pstMsg == NULL)
    {
        p_debug("realloc to %d bytes failed",sizeof(MsgHeader) + nLen);
        free(pstMsg);
        return MSGRET_RESOURCE_EXCEEDED;
    }
    memcpy((SINT8 *)(pstMsg + 1), pBuf, nLen);
...
SINT32 DM_UdpReceive(SINT32 iFd, SINT8 **pBuf, UINT32 *pnTimeOut,struct sockaddr_in *addr)
{
    ...
    if (pstMsg == NULL)
    {
        p_debug("alloc of msg header failed");
        return MSGRET_RESOURCE_EXCEEDED;
    }
...
//    p_debug("pInBuf = %s ,iRc = %d ,errno = %d",pInBuf, iRc, errno);
    *pBuf = pInBuf;
	free(pstMsg);
    return iRc;
}
...
SINT32 DM_UdpClientInit(SINT32 iDomain, SINT32 iPort, SINT32 iType,SINT8 *ipaddr,struct sockaddr_in *stServerAddr)
{
    ...
    if (iFd < 0)
    {
        p_debug("Could not create socket");
        return -1;
    }	
...
SINT32 DM_UdpServerInit(SINT32 iDomain, SINT32 iPort, SINT32 iType, SINT32 iBackLog) 
{
    ...
    if (iType == SOCK_STREAM)
    {
        ...
        {
            p_debug("listen errno=%d port=%d fd=%d reason:%s", errno, iPort, iFd, strerror(errno));
            close(iFd);
            return MSG_INVALID_FD;
        }
    }
