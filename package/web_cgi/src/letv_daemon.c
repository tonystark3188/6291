...
int isPhoneConnected( char *mode){
	...
	if(strcmp(mode,"ap")==0){
		sprintf(tmpStr,"%s","wl -i wl0.1 assoclist |wc -l");
	}	
	else if (strcmp(mode,"sta") == 0)
	{
		sprintf(tmpStr,"%s","wl -i p2p-p2p0-0 assoclist |wc -l");
	}
	read_fp=popen(tmpStr,"r");
	if(read_fp!=NULL)
	{
		chars_read = fread(buffer, sizeof(char), BUFSIZ-1, read_fp); 
		if (chars_read > 0){
			current_sta_cnt=buffer[0]-'0';
			p_debug("current_sta_cnt=%d",current_sta_cnt);
			pclose(read_fp);
		}else {
			p_debug("chars_read=%d",chars_read);
			pclose(read_fp);
...
int isClientConnected(char *mode){
	...
	if(strcmp(mode,"ap")==0){
		sprintf(tmpStr,"%s","wl -i wlan0 assoclist |wc -l");
	}	
	read_fp=popen(tmpStr,"r");
	if(read_fp!=NULL)
	{
		chars_read = fread(buffer, sizeof(char), BUFSIZ-1, read_fp); 
		if (chars_read > 0){
			client_status=buffer[0]-'0';
			p_debug("client_status=%d",client_status);
			pclose(read_fp);
		}else {
			p_debug("chars_read=%d",chars_read);
			pclose(read_fp);
...
#define LETV_SERVER_IP 		"www.qq.com"
...
int isInternetOK( char *wifimode){
	...
	system("echo default-on > /sys/class/leds/led\:wifi\:green/trigger;echo none > /sys/class/leds/led\:wifi\:green/trigger;echo none > /sys/class/leds/led\:wifi\:blue/trigger;");
	return 0;
...
void main()
{
	...
	int wifi_mode_btn=0;
	int cur_wifi_mode_btn=0;
	wifi_mode_btn=check_button();
	getTimeOfDay();
	char led_status[8]="\0";
	while(1)
		{
		...
		if(!strcmp(start_upgrade,"1")){//
			...
			cur_wifi_mode_btn=check_button();
			p_debug("cur_wifi_mode_btn=%d",cur_wifi_mode_btn);
			if(cur_wifi_mode_btn != wifi_mode_btn){
				wifi_mode_btn=cur_wifi_mode_btn;
				p_debug("wifi_mode_btn=%d",wifi_mode_btn);
				if(strcmp(wifimode,"sta")==0){
					system("sh /sbin/sw.sh sta &");
				}
				else
...
//						system("mcu_control -s 1");//on
	...
	//////////////////////
			...
			if(check_internet>10){
				p_debug("check_internet=%d",check_internet);
				if(!strcmp(wifimode,"ap")){
					if((net=isInternetOK(wifimode))<0)
					{				p_debug("led 3");
						system("echo default-on > /sys/class/leds/led\:wifi\:red/trigger; \
							echo gpio > /sys/class/leds/led\:wifi\:green/trigger;echo 0 > /sys/class/leds/led\:wifi\:green/brightness; \
							echo gpio > /sys/class/leds/led\:wifi\:blue/trigger;echo 0 > /sys/class/leds/led\:wifi\:blue/brightness;");	
						printf("internet is not ok.\n");
						...
						system("echo default-on > /sys/class/leds/led\:wifi\:green/trigger; \
							echo gpio > /sys/class/leds/led\:wifi\:red/trigger;echo 0 > /sys/class/leds/led\:wifi\:red/brightness;\
							echo gpio > /sys/class/leds/led\:wifi\:blue/trigger;echo 0 > /sys/class/leds/led\:wifi\:blue/brightness;");	
					}
				}else if(!strcmp(wifimode,"sta")){
					updateSysVal("net","false");
					system("echo default-on > /sys/class/leds/led\:wifi\:blue/trigger; \
						echo gpio > /sys/class/leds/led\:wifi\:red/trigger; echo 0 > /sys/class/leds/led\:wifi\:red/brightness;\
						echo gpio > /sys/class/leds/led\:wifi\:green/trigger;echo 0 > /sys/class/leds/led\:wifi\:green/brightness;");						
...
//					system("mcu_control -s 1");
			...
			if(PhoneConnected ){
				get_conf_str(led_status,"led_status");
				if(!strcmp(led_status,"3")||!strcmp(led_status,"1")){
					//system("mcu_control -s 1");
					//system("pwm_control 1  1 0;pwm_control 1 0 0");		
					p_debug("led 2");
					system("echo default-on > /sys/class/leds/led\:sys\:blue/trigger; \
				...
				get_conf_str(led_status,"led_status");
				p_debug("led ");
				if(strcmp(led_status,"3")&&strcmp(led_status,"2")){
//						system("mcu_control -s 3");
						system("echo timer > /sys/class/leds/led\:sys\:blue/trigger;echo gpio > /sys/class/leds/led\:sys\:green/trigger;echo 0 > /sys/class/leds/led\:sys\:green/brightness;");
						updateSysVal("led_status","3");
					}

			}
			if(sleep_time>1800)//30 minutes
				{
				...
				//system("uart_client_test -s 3");// force power off
				//
				
				system("ifconfig wlan0 down >/dev/null");//close wifi
				updateSysVal("w_m","0");
			}
...
system("echo timer > /sys/class/leds/led\:sys\:green/trigger;echo gpio > /sys/class/leds/led\:sys\:blue/trigger;echo 0 > /sys/class/leds/led\:sys\:blue/brightness;");	
		system("upgrade.sh >/dev/null");
